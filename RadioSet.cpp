	...
	: TForm(AOwner)
{
	...
	if( MsgEng ){
		...
		Caption = "リグコントロール(Comando de radio)";
		GB1->Caption = "ポートの設定";
    }
	int i;
...
//
	...
	if( MsgEng ){
		pBox->Title = "Abrir archivo de comandos";
...
void __fastcall TRADIOSetDlg::SaveBtnClick(TObject *Sender)
{
	...
	if( MsgEng ){
		pBox->Title = "Guardar archivo de comandos";
		pBox->Filter = "Command files(*.rcm)|*.rcm|";
	}
	else {
...
void __fastcall TRADIOSetDlg::RefBtnClick(TObject *Sender)
{
	...
	pBox->Options << ofFileMustExist;
	if( MsgEng ){
		pBox->Title = "Archivo de programa";
