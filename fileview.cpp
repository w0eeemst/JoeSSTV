	...
	: TForm(Owner)
{
	...
	if( MsgEng ){
		SBNew->Hint = "Cambiar Carpeta";
		SBUpdate->Hint = "Actualizar archivos recientes";
		SBMode->Hint = "Copia el·stica";

		KTXS->Caption = "&Copia el·stica a TX";
		KTX->Caption = "Copia a TX (via Clipper)...";
		KPO->Caption = "Superponer a plantilla";
		KPOL->Caption = "Incrustar en plantilla como OLE";
//		KPE->Caption = "&Edit...";
		KTT->Caption = "Mostrar barra de tÌtulo";
		KTK->Caption = "Mostrar la imagen como manteniendo la relaciÛn de aspecto";
		KNP->Caption = "Crear nueva p·gina";
		KDP->Caption = "Eliminar p·gina";
		KRP->Caption = "Renombrar p·gina";
		KBP->Caption = "PestaÒa inferior";
		KPI->Caption = "Acceso r·pido con Ìndice";
	}
...
void __fastcall TFileViewDlg::UpdateStat(void)
{
	...
	sprintf(bf, "%u/%u (%u archivos)", UD->Position + 1, pCurPage->m_MaxPage, FileCnt);
	LPage->Caption = bf;
	UpdateTitle();
}
//---------------------------------------------------------------------------
...
void __fastcall TFileViewDlg::PBDblClick(TObject *Sender)
{
	...
	if( pCurPage->m_Type ){     // ÉeÉìÉvÉåÅ[Ég
		CWaitCursor w;
...
int __fastcall TFileViewDlg::NewFolder(void)
{
	...
	if( MsgEng ){
		pDialog->Title = "Establecer carpeta";
	}
	else {
		pDialog->Title = "ÉtÉHÉãÉ_ÇÃëIë";
	}
	char bf[512];
...
void __fastcall TFileViewDlg::PopupPopup(TObject *Sender)
{
	...
	switch(pCurPage->m_Type){
		case 1:
			KP->Caption = MsgEng ? "Copiar a plantilla" : "ÉeÉìÉvÉåÅ[ÉgÇ…ÉRÉsÅ[";
			break;
		default:
			KP->Caption = MsgEng ? "Copiar a plantilla" : "ÉeÉìÉvÉåÅ[ÉgÇ…ì\ÇËïtÇØ";
			break;
	}
...
void __fastcall TFileViewDlg::KDClick(TObject *Sender)
{
	...
	AnsiString as;
	GetCurFileName(as);
	if( YesNoMB( ( Font->Charset != SHIFTJIS_CHARSET )?"Eliminar '%s'":"'%s'Çè¡ãéÇµÇ‹Ç∑", as.c_str() ) == IDYES ){
		if( unlink(as.c_str()) ){
...
void __fastcall TFileViewDlg::KDPClick(TObject *Sender)
{
	...
	if( YesNoMB( MsgEng ? "Eliminar '%s', øest·s seguro?" : "'%s'ÇçÌèúÇµÇ‹Ç∑. ÇÊÇÎÇµÇ¢Ç≈Ç∑Ç©?", as.c_str()) != IDYES ) return;

	m_CurFile = -1;
	pFileV[m_CurPage]->Delete();
	if( m_CurPage < (m_MaxPage - 1) ){  // ìríÜÇÃÉyÅ[ÉW
		int i;
...
void __fastcall TFileViewDlg::KRPClick(TObject *Sender)
{
	...
	if( InputMB(NULL, MsgEng ? "Ingrese el nombre de la p·gina" : "ñºëOÇì¸óÕ", as) == TRUE ){
