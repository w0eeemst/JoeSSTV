	...
	: TForm(AOwner)
{
	...
	if( MsgEng ){
		Caption = "Definir Conversi";
		ConvSel->Caption = "Delimitador de Tipo";
		ConvSel->Items->Strings[0] = "coma(,)";
		ConvSel->Items->Strings[1] = "TAB";
		ConvSel->Items->Strings[2] = "NO";
		LConv->Caption = "Conv.";
		CheckDBL->Caption = "Doble";
		GrpRef->Caption = "Ref.";
		SBBack->Caption = "<";
		SBNext->Caption = ">";
		CancelBtn->Caption = "Cancelar";
		InitBtn->Caption = "Borra todo";
		UpBtn->Caption = "Arriba";
		DownBtn->Caption = "Abajo";
...
{
	if( YesNoMB( MsgEng ? "Eliminar toda la configuraci. ｿEst疽 seguro?":"すべての定義を消去しますか？" ) == IDYES ){
...
{
	if( YesNoMB( MsgEng ? "Eliminar toda la longitud. ｿEst疽 seguro?":"すべての文字列長の制限を無くしますか？" ) == IDYES ){
		for( int i = 0; i < TEXTCONVMAX; i++ ){
			Conv[i].w = 0;
		}
		Grid->Invalidate();
