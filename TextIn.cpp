	...
	: TForm(AOwner)
{
	...
	if( MsgEng ){
		Caption = "Texto & Colores";
		CancelBtn->Caption = "Cancelar";
		CmdBtn->Caption = "Macro";
		Ltext->Caption = "Texto";
		GBEf->Caption = "";
		RGGrade->Caption = "Gradación";
		RGGrade->Items->Strings[0] = "NO";
		RGGrade->Items->Strings[1] = "Horizontal";
		RGGrade->Items->Strings[2] = "Vertical";
		RGGrade->Items->Strings[3] = "Máscara Bitmap";
		RGShadow->Caption = "Sombra";
		RGShadow->Items->Strings[0] = "NO";
		RGShadow->Items->Strings[1] = "Borde";
		RGShadow->Items->Strings[2] = "Sombra 1";
		RGShadow->Items->Strings[3] = "Sombra 2";
		RGShadow->Items->Strings[4] = "Sombra 3";
		RGShadow->Items->Strings[5] = "Sombra 4";
		RGShadow->Items->Strings[6] = "Sombra 5";
		RGShadow->Items->Strings[7] = "Sombra 6";
		TblBtn->Caption = "Componentes";
		MaskBtn->Caption = "Bitmap";
		if( Font->Charset == HANGEUL_CHARSET ){
			CBZero->Caption = "Slash zero (\xa8\xaa)";
		}
		else {
			CBZero->Caption = "Slash zero (Ø)";
		}
		LRot->Caption = "Rotación";
		CBRA->Caption = "Justificar a derecha";
		CBTF->Caption = "Perspectiva";
		GBP->Caption = "Perfiles";
		MemBtn->Caption = "Agregar";
		DelBtn->Caption = "Borrar";
		CBFont->Caption = "Guardar fuente";
		CBV->Caption = "Vert";
		FontBtn->Caption = "Fuente";
		FontBtn->Hint = "Elige una fuente";
		SBFD->Hint = "Haz la fuente más pequeña";
		SBFI->Hint = "Haz la fuente más grande";
        SBB->Hint = "Bold";
        SBI->Hint = "Italic";
		SBMSG->Hint = "Mostrar lista de mensajes";
	}
...
void __fastcall TTextInDlg::UpdatePara(CDrawText *pm)
{
	...
	pm->m_Text = EditText->Text.IsEmpty() ? "TEXTO" : AnsiString(EditText->Text).c_str();	//ja7ude 0521
	pm->m_Col1 = PC1->Color;
	pm->m_Col2 = PC2->Color;
	pm->m_Col3 = PC3->Color;
	pm->m_Col4 = PC4->Color;
...
int __fastcall TTextInDlg::Execute(CDrawText *p)
{
	...
	m_Text.Start(PBox->Canvas, 0, 0);       // ƒLƒƒƒ“ƒoƒX‚ÌƒZƒbƒg
	m_Text.Copy(pItem, 0);
//    m_Text.UpdateText();
	...
	if( pDialog->Execute() == TRUE ){
		...
		if( (Sender == PC1)||
			...
			if( (PC1->Color == PC2->Color) && (PC3->Color == PC4->Color) ){
				if( YesNoMB(MsgEng ? "¿Cambiar a gradación de dos colores?" : "2F‚ÌƒOƒ‰ƒf[ƒVƒ‡ƒ“‚É•ÏX‚µ‚Ü‚·‚©?" ) == IDYES ){
...
void __fastcall TTextInDlg::InitCMB(void)
{
	...
	CMBPro->Text = MsgEng ? "Sin título" : "–³‘è";
...
void __fastcall TTextInDlg::UpdateSBF(void)
{
	...
	for( int i = 0; i < 4; i++, pData++ ){
		if( pData->m_Height ){
			...
			as = MsgEng ? "Cargar fuente registrada" : "“o˜^Ì«ÝÄ‚Ì“Ç‚Ýo‚µ";
        }
		as += MsgEng ? " (Registrar = Botón derecho)" : " (“o˜^=‰EÎÞÀÝ)";
...
void __fastcall TTextInDlg::AddStyle(AnsiString &as, BYTE charset, DWORD style)
{
    switch(charset){
		case ANSI_CHARSET:
        	...
        	break;
		case SHIFTJIS_CHARSET:
			as += MsgEng ? "/Japonés" : "/“ú–{Œê";
        	break;
		case HANGEUL_CHARSET:
			as += MsgEng ? "/Koreano(Hangul)" : "/ƒnƒ“ƒOƒ‹";
			break;
		case CHINESEBIG5_CHARSET:   // ‘ä˜p
			as += MsgEng ? "/Chino" : "/’†‘Œê(‘ä˜p)";
			break;
		case 134:                   // ’†‘ŒêŠÈ—ª
			as += MsgEng ? "/Chino" : "/’†‘Œê(ŠÈ—ª)";
			break;
        default:
        	break;
    }
	if( style & FSBOLD ) as += MsgEng ? "/Bold" : "/‘¾Žš";
	if( style & FSITALIC ) as += MsgEng ? "/Italic" : "/ŽÎ‘Ì";
	if( style & FSUNDERLINE ) as += MsgEng ? "/Subrayar" : "/‰ºü";
	if( style & FSSTRIKEOUT ) as += MsgEng ? "/Tachar" : "/ŽæÁ‚µü";
}
//---------------------------------------------------------------------------
