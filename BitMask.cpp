	...
	: TForm(AOwner)
{
	...
	if( MsgEng ){
		Caption = "Máscara de Bitmap";
		CancelBtn->Caption = "Cancelar";
		L1->Caption = "Tamaño";
		GB2->Caption = "Crear y editar máscara";
		PaintBtn->Caption = "Editar";

		SBPaste->Hint = "Pegar máscara desde el portapapeles";
		SBCopy->Hint = "Copiar máscara en el portapapeles";
		SBColAdj->Hint = "Ajusta la imagen de la máscara";
		GB1->Caption = "Máscara de definición de usuario";
		L3->Caption = "Arrastrar&&Soltar para registrar";
	}
...
{
	m_Text.Start(PBoxText->Canvas, 0, 0);       // ƒLƒƒƒ“ƒoƒX‚ÌƒZƒbƒg
	m_Text.Copy(pText);
	m_Text.UpdateText();
...
void __fastcall TBitMaskDlg::LoadBtnClick(TObject *Sender)
{
	...
	if( MsgEng ){
		pDialog->Title = "Cargar imagen del archivo";
	}
	else {
