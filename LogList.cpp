...
void __fastcall TLogListDlg::UpdateLogCount(int sw)
{
	...
	if( sw ){
		...
		char bf[256];
		if( Log.IsOpen() ){
			sprintf(bf, MsgEng ? "%s - [%u QSO (s)]" : "%s - [%uåèÇÃÉfÅ[É^Ç™Ç†ÇËÇ‹Ç∑]", Log.m_FileName.c_str(), Log.GetCount());
        }
        else {
...
{
	if( YesNoMB( MsgEng ? "Eliminar selecciÛn. (No podr· restaurar) øEst· seguro?":"åªç›ëIëÇ≥ÇÍÇƒÇ¢ÇÈîÕàÕÇçÌèúÇµÇ‹Ç∑.çÌèúÇµÇΩì‡óeÇÕïúå≥Ç≈Ç´Ç‹ÇπÇÒ\r\n\r\nÇÊÇÎÇµÇ®Ç‹Ç¡Ç©ÅH") == IDYES ){
...
void __fastcall TLogListDlg::KFindTopClick(TObject *Sender)
{
	if( m_FindCall.IsEmpty() || (Sender == KFindTop) ){
		...
		if( InputMB(NULL, MsgEng ? "Indicativo":"åüçıÇ∑ÇÈÉRÅ[ÉãÉTÉCÉì", as) == FALSE ) return;
		jstrupr(as.c_str());
		m_FindCall = as;
	}
	...
	if( n >= 0 ){
		...
		WarningMB(MsgEng ? "'%s' no fue encontrado" : "'%s'ÇÕå©Ç¬Ç©ÇËÇ‹ÇπÇÒÇ≈ÇµÇΩ.", m_FindCall.c_str());
	}
}
//---------------------------------------------------------------------------
...
{
	if( YesNoMB( MsgEng ? "Puede llevar mucho tiempo. øEst·s seguro?":"Ç±ÇÃèàóùÇÕÉÅÉ`ÉÉÉìÉRéûä‘Ç™Ç©Ç©ÇÈÇ©Ç‡ímÇÍÇ‹ÇπÇÒ.\r\n\r\nÇÊÇÎÇµÇ®Ç‹Ç¡Ç©ÅH" ) == IDYES ){
		int top = Grid->Selection.Top - 1;
		int end = Grid->Selection.Bottom - 1;
		if( top == end ){
			top = 0;
...
//---------------------------------------------------------------------------
// ÇlÇlÇkÇnÇfÉeÉLÉXÉgÉtÉ@ÉCÉãÇÃÉçÅ[Éh
void __fastcall TLogListDlg::KMTextReadClick(TObject *Sender)
{
	...
	if( MsgEng ){
		...
		OpenDialog->Title = "MMLOGÉeÉLÉXÉgÉtÉ@ÉCÉãÇÃÉçÅ[Éh";
		OpenDialog->Filter = "MMLOGÉeÉLÉXÉgÉtÉ@ÉCÉã(*.log)|*.log|";
	}
	OpenDialog->FileName = "";
...
void __fastcall TLogListDlg::KMTextWriteClick(TObject *Sender)
{
	...
	if( MsgEng ){
		...
		SaveDialog->Title = "MMLOGÉeÉLÉXÉgÉtÉ@ÉCÉãÇÃçÏê¨";
		SaveDialog->Filter = "MMLOGÉeÉLÉXÉgÉtÉ@ÉCÉã(*.log)|*.log|";
	}
	char bf[256];
...
int __fastcall TLogListDlg::SureRead(void)
{
	int r;
	if( MsgEng ){
		r = YesNoCancelMB("MMSSTV ya tiene datos de QSO. øAgregar datos QSO del archivo?\r\n\r\n<Yes>\tAÒadido al final\r\n<No>\tSobreinscribir\r\n\t(Todos los datos actuales se eliminan y se reemplazan por los datos leÌdos)\r\n<Cancel>\tAbortar");
	}
	else {
		r = YesNoCancelMB("åªç›MMSSTVÇ…ÉçÉOÉfÅ[É^Ç™ë∂ç›ÇµÇ‹Ç∑. í«â¡ì«Ç›Ç±Ç›ÇµÇ‹Ç∑Ç©ÅH\r\n\r\n<ÇÕÇ¢>\tåªç›ÇÃÉçÉOÉfÅ[É^ÇÃç≈å„Ç…ì«Ç›çûÇÒÇæÉfÅ[É^Ç™í«â¡Ç≥ÇÍÇÈ.\r\n<Ç¢Ç¢Ç¶>\tåªç›ÇÃÉçÉOÉfÅ[É^ÇÕëSÇƒçÌèúÇ≥ÇÍì«Ç›çûÇÒÇæÉfÅ[É^Ç…íuÇ´ä∑ÇÌÇÈ.\r\n<∑¨›æŸ>\tèàóùíÜé~");
	}
	switch(r){
		case IDCANCEL:
			return 0;
		case IDNO:
			if(  MsgEng ){
				r = OkCancelMB("Eliminar todos los datos de QSO ... (no podr· restaurar) øEst· seguro?");
			}
			else {
				r = OkCancelMB("åªç›MMSSTVÇ™ï€éùÇµÇƒÇ¢ÇÈÉçÉOÉfÅ[É^ÇëSÇƒé∏Ç¢Ç‹Ç∑.\r\n\r\nÇŸÇÒÇ‹Ç…Ç¶Ç¶Ç≈Ç¡Ç©ÅH");
			}
			switch(r){
				case IDOK:
					Log.DeleteAll();
...
void __fastcall TLogListDlg::LoadMmlogText(LPCSTR pName)
{
	...
	if( fp != NULL ){
		int r = IDNO;
		if( MsgEng ){
			r = YesNoMB("Esta funciÛn existe para la compatibilidad con software antiguo que maneja JST.\r\n\r\nøLos datos grabados son UTC?");
		}
		CWaitCursor w;
		SDMMLOG	sd;
		while(!feof(fp)){
		...
		ErrorMB(MsgEng ? "No se puede abrir '%s'" : "'%s'Ç™å©Ç¬Ç©ÇËÇ‹ÇπÇÒ.", pName);
	}
	UpdateLogCount(1);
}
//---------------------------------------------------------------------------
void __fastcall TLogListDlg::SaveMmlogText(LPCSTR pName)
{
	...
	if( fp != NULL ){
		...
		if( fclose(fp) ){
			ErrorMB( MsgEng ? "No se puede guardar '%s'" : "'%s'Ç™ê≥ÇµÇ≠çÏê¨Ç≈Ç´Ç‹ÇπÇÒÇ≈ÇµÇΩ.", pName);
		}
	}
	else {
		ErrorMB(MsgEng ? "No se puede guardar '%s'" : "'%s'Ç™çÏê¨Ç≈Ç´Ç‹ÇπÇÒ.", pName);
	}
}
//---------------------------------------------------------------------------
...
void __fastcall TLogListDlg::KExportTextClick(TObject *Sender)
{
	...
	if( MsgEng ){
		...
		SaveDialog->Filter = "ÉeÉLÉXÉgÉtÉ@ÉCÉã(*.csv;*.txt)|*.csv;*.txt|";
	}
	char bf[256];
	strcpy(bf, Log.GetName());
	SetEXT(bf, ".csv");
...
void __fastcall TLogListDlg::KImportTextClick(TObject *Sender)
{
	...
	if( MsgEng ){
		...
		OpenDialog->Title = "ÉeÉLÉXÉgÉtÉ@ÉCÉãÇÃÉçÅ[Éh";
		OpenDialog->Filter = "ÉeÉLÉXÉgÉtÉ@ÉCÉã(*.csv;*.txt)|*.csv;*.txt|";
	}
	OpenDialog->FileName = "";
...
void __fastcall TLogListDlg::KExportLog200Click(TObject *Sender)
{
	...
	if( MsgEng ){
		...
		SaveDialog->Title = "LOG200ÉfÅ[É^ÉtÉ@ÉCÉãÇÃçÏê¨";
		SaveDialog->Filter = "LOG200ÉfÅ[É^ÉtÉ@ÉCÉã(*.l2)|*.l2|";
	}
	char bf[256];
	strcpy(bf, Log.GetName());
	SetEXT(bf, ".l2");
...
void __fastcall TLogListDlg::KImportLog200Click(TObject *Sender)
{
	...
	if( MsgEng ){
		...
		OpenDialog->Title = "LOG200ÉfÅ[É^ÉtÉ@ÉCÉãÇÃÉçÅ[Éh";
		OpenDialog->Filter = "LOG200ÉfÅ[É^ÉtÉ@ÉCÉã(*.l2)|*.l2|";
	}
	OpenDialog->FileName = "";
...
int __fastcall TLogListDlg::SaveHamLog5(LPCSTR pName)
{
	...
	CHamlog5 HamLog;
    if( HamLog.Open(master, FALSE) ){	// ÉtÉBÅ[ÉãÉhèÓïÒÇì«Ç›çûÇﬁ
    	HamLog.Close();
    }
    else {								// ÉtÉBÅ[ÉãÉhèÓïÒÇñ‚Ç¢çáÇÌÇπ
...
void __fastcall TLogListDlg::KImportHamlogClick(TObject *Sender)
{
	...
	if( MsgEng ){
		...
		OpenDialog->Title = "HamLogÉfÅ[É^ÉtÉ@ÉCÉãÇÃÉçÅ[Éh";
		OpenDialog->Filter = "HamLogÉfÅ[É^ÉtÉ@ÉCÉã(*.hdb;*.dbs)|*.hdb;*.dbs|";
	}
	OpenDialog->FileName = "";
...
void __fastcall TLogListDlg::KExportHamlogClick(TObject *Sender)
{
	...
	if( MsgEng ){
		...
		SaveDialog->Title = "HAMLOGÉfÅ[É^ÉtÉ@ÉCÉãÇÃçÏê¨";
		SaveDialog->Filter = "HAMLOG (Ver5) ÉfÅ[É^ÉtÉ@ÉCÉã(*.hdb)|*.hdb|HAMLOGÉfÅ[É^ÉtÉ@ÉCÉã(*.dbs)|*.dbs|";
	}
	char bf[256];
	strcpy(bf, Log.GetName());
	SetEXT(bf, ".hdb");
...
void __fastcall TLogListDlg::KExportADIFClick(TObject *Sender)
{
	...
	if( MsgEng ){
		...
		SaveDialog->Title = "ADIF ÉtÉ@ÉCÉãÇÃçÏê¨";
		SaveDialog->Filter = "ADIF ÉtÉ@ÉCÉã(*.adi)|*.adi|";
	}
	char bf[256];
	strcpy(bf, Log.GetName());
	SetEXT(bf, ".adi");
...
void __fastcall TLogListDlg::KImportADIFClick(TObject *Sender)
{
	...
	if( MsgEng ){
		...
		OpenDialog->Title = "ADIF ÉtÉ@ÉCÉãÇÃÉçÅ[Éh";
		OpenDialog->Filter = "ADIF ÉtÉ@ÉCÉã(*.adi;*.txt)|*.adi;*.txt|";
	}
	OpenDialog->FileName = "";
...
void __fastcall TLogListDlg::KExportCabrilloClick(TObject *Sender)
{
	...
	if( MsgEng ){
		...
		SaveDialog->Title = "Cabrillo ÉtÉ@ÉCÉãÇÃçÏê¨";
		SaveDialog->Filter = "Cabrillo ÉtÉ@ÉCÉã(*.txt)|*.txt|";
	}
	char bf[256];
	strcpy(bf, Log.GetName());
	SetEXT(bf, ".txt");
