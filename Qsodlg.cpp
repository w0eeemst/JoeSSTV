...
//	Grid->TopRow = 1;
	...
	SetCountry();
	char bf[512];
	strcpy(bf, MsgEng ? "Datos iniciales":"QSOデータ初期値");
...
void __fastcall TQSODlgBox::UpdateStat(void)
{
	...
	if( MsgEng ){
		if( !cmp2max ){
			strcpy(bf, "No hay datos");
		}
		else if( cmp1max != cmp2max ){
			sprintf(bf, "Perfecto: %u  Misma llamada: %u", cmp1max, cmp2max - cmp1max);
		}
		else {
			sprintf(bf, "Perfecto: %u", cmp1max );
		}
	}
	else {
		if( !cmp2max ){
	  ...
	  TRect &Rect, TGridDrawState State)
{
	...
	if( Row ){
		...
		LPCSTR	_tt[]={
			"Fecha","JST","Indic","M", "Env","Rcb","Banda","Modo","Pow","Nombre","QTH","S","R",
			"Nota","QSL", "End", "Env", "Opt1", "Opt2", "Usr1", "Usr2",
		};
		if( Col == 1 ){
