...
#define FSKIDMAX    32//Conectar a omni-rig
AnsiString __fastcall TMmsstv::adif(AnsiString tipo, AnsiString dato)
{
AnsiString salida;
int longitud;
longitud = dato.Length();
salida = "<"+tipo+":"+IntToStr(longitud)+">"+dato+" ";
return salida;
}
//--------------------------------------------------------------------------
int __fastcall TMmsstv::porcentaje(int max, int valor)
{
int regla = (100*valor)/max;
return regla;
}
//--------------------------------------------------------------------------
AnsiString __fastcall TMmsstv::qrzkey(AnsiString datos)
{
AnsiString key;
int inic, fina;
inic = datos.Pos("<Key>");
fina = datos.Pos("</Key>");
key = datos.SubString(inic+5,fina-inic-5);
return key;
}
//--------------------------------------------------------------------------
AnsiString __fastcall TMmsstv::Frecuencia(long frec)
{
AnsiString frecuencia, mhz, khz, hz;

if ((frec>=1810000)&&(frec<=2700000)) {
	frecuencia = FloatToStr(frec);
	mhz = frecuencia.SubString(1,1);
	khz = frecuencia.SubString(2,3);
	hz = frecuencia.SubString(5,3);
	return " "+mhz+"."+khz+"Khz->160M";
}else if ((frec>=3500000)&&(frec<=3800000)) {
	frecuencia = FloatToStr(frec);
	mhz = frecuencia.SubString(1,1);
	khz = frecuencia.SubString(2,3);
	hz = frecuencia.SubString(5,3);
	return " "+mhz+"."+khz+"Khz->80M";
}else if ((frec>=5351500)&&(frec<=5366500)) {
	frecuencia = FloatToStr(frec);
	mhz = frecuencia.SubString(1,1);
	khz = frecuencia.SubString(2,3);
	hz = frecuencia.SubString(5,3);
	return " "+mhz+"."+khz+"Khz->60M";
}else if ((frec>=7000000)&&(frec<=7200000)) {
	frecuencia = FloatToStr(frec);
	mhz = frecuencia.SubString(1,1);
	khz = frecuencia.SubString(2,3);
	hz = frecuencia.SubString(5,3);
	return " "+mhz+"."+khz+"Khz->40M";
}else if ((frec>=10000000)&&(frec<=10150000)) {
	frecuencia = FloatToStr(frec);
	mhz = frecuencia.SubString(1,2);
	khz = frecuencia.SubString(3,3);
	hz = frecuencia.SubString(6,3);
	return " "+mhz+"."+khz+"Khz->30M";
}else if ((frec>=14000000)&&(frec<=14350000)) {
	frecuencia = FloatToStr(frec);
	mhz = frecuencia.SubString(1,2);
	khz = frecuencia.SubString(3,3);
	hz = frecuencia.SubString(6,3);
	return " "+mhz+"."+khz+"Khz->20M";
}else if ((frec>=18068000)&&(frec<=18168000)) {
	frecuencia = FloatToStr(frec);
	mhz = frecuencia.SubString(1,2);
	khz = frecuencia.SubString(3,3);
	hz = frecuencia.SubString(6,3);
	return " "+mhz+"."+khz+"Khz->17M";
}else if ((frec>=21000000)&&(frec<=21450000)) {
	frecuencia = FloatToStr(frec);
	mhz = frecuencia.SubString(1,2);
	khz = frecuencia.SubString(3,3);
	hz = frecuencia.SubString(6,3);
	return " "+mhz+"."+khz+"Khz->15M";
}else if ((frec>=24890000)&&(frec<=24990000)) {
	frecuencia = FloatToStr(frec);
	mhz = frecuencia.SubString(1,2);
	khz = frecuencia.SubString(3,3);
	hz = frecuencia.SubString(6,3);
	return " "+mhz+"."+khz+"Khz->12M";
}else if ((frec>=26965000)&&(frec<=27405000)) {
	frecuencia = FloatToStr(frec);
	mhz = frecuencia.SubString(1,2);
	khz = frecuencia.SubString(3,3);
	hz = frecuencia.SubString(6,3);
	return " "+mhz+"."+khz+"Khz->11M";
}else if (frec==27700000) {
	frecuencia = FloatToStr(frec);
	mhz = frecuencia.SubString(1,2);
	khz = frecuencia.SubString(3,3);
	hz = frecuencia.SubString(6,3);
	return " "+mhz+"."+khz+"Khz->11M";
}else if ((frec>=28000000)&&(frec<=29700000)) {
	frecuencia = FloatToStr(frec);
	mhz = frecuencia.SubString(1,2);
	khz = frecuencia.SubString(3,3);
	hz = frecuencia.SubString(6,3);
	return " "+mhz+"."+khz+"Khz->10M";
}else if ((frec>=50000000)&&(frec<=52000000)) {
	frecuencia = FloatToStr(frec);
	mhz = frecuencia.SubString(1,2);
	khz = frecuencia.SubString(3,3);
	hz = frecuencia.SubString(6,3);
	return " "+mhz+"."+khz+"Khz->6M";
}else if ((frec>=70150000)&&(frec<=70200000)) {
	frecuencia = FloatToStr(frec);
	mhz = frecuencia.SubString(1,2);
	khz = frecuencia.SubString(3,3);
	hz = frecuencia.SubString(6,3);
	return mhz+"."+khz+"Khz->4M";
}else if ((frec>=144000000)&&(frec<=146000000)) {
	frecuencia = FloatToStr(frec);
	mhz = frecuencia.SubString(1,3);
	khz = frecuencia.SubString(4,3);
	hz = frecuencia.SubString(7,3);
	return " "+mhz+"."+khz+"Khz->2M";
}else if ((frec>=430000000)&&(frec<=440000000)) {
	frecuencia = FloatToStr(frec);
	mhz = frecuencia.SubString(1,3);
	khz = frecuencia.SubString(4,3);
	hz = frecuencia.SubString(7,3);
	return " "+mhz+"."+khz+"Khz->70CM";
}else return " FUERA DE BANDA";
}
AnsiString __fastcall TMmsstv::Frecuencia_Sola(long frec)
{
AnsiString frecuencia, mhz, khz;
...
AnsiString __fastcall TMmsstv::Banda(long frec)
{
AnsiString frecuencia, mhz, khz, hz;
...
void __fastcall TMmsstv::ConectaOmnirig(void)
{
//Creamos objeto
 try{
	OmniRig = new TOmniRigX(this);
	Timer1->Enabled = true;
	OmniRig->Connect();
	omni = "1";
	Memo2->Lines->Values["omnirig"]=1;
	}catch(...){
			   Timer1->Enabled = false;
			   delete OmniRig;
			   omni = "0";
			   ShowMessage(L"No se ha podido conectar con Omni-Rig");
			   }
	//delete OmniRig;
	//OmniRig = NULL;
Application->ProcessMessages();
}
//Desconectar a omni-rig
void __fastcall TMmsstv::DesconectaOmnirig(void)
{
omni = "0";
...
//Abre configuraciÛn Omni-Rig
void __fastcall TMmsstv::ConfiguraOmnirig(void)
{
...
void __fastcall TMmsstv::WndCopyData(TMessage &Message)
{
	...
	switch(cp->dwData){
		...
		case 0x80001212:    // é¸îgêîÉfÅ[É^ÇÃéwíË
			...
			Message.Result = TRUE;
			break;
		case 0x80001213:    // ÉNÉäÉbÉvÉ{Å[ÉhÇ©ÇÁÇÃÉçÅ[Éh
			KESClick(NULL);
			Message.Result = TRUE;
			break;
		case 0x80001214:    // âÊëúÉTÉCÉYÇÃñ‚Ç¢çáÇÌÇπ
			Message.Result = m_TXW + (m_TXH << 16);
			break;
		case 0x80001215:    // óöóâÊëúÇÃÉRÉsÅ[
			SBCopyClick(NULL);
			Message.Result = TRUE;
			break;
		case 0x80001216:    // éÛêMâÊëúÇÃÉRÉsÅ[
			KERClick(NULL);
			Message.Result = TRUE;
			break;
		...
		case 0x80001218:
			...
			Message.Result = TRUE;
			break;
		case 0x80001219:    // REMÉfÅ[É^ÇÃéwíË
			if( cp->cbData && (cp->lpData != NULL) ){
...
if (error != 10061) {
	if( m_AppErr < 3 ){
		...
		ErrorMB(MsgEng?
			...
			"å¥àˆïsñæÇÃÉGÉâÅ[Ç™î≠ê∂ÇµÇ‹ÇµÇΩ."
		);
	}
	ResDisPaint();
	}
...
//    sys.m_VOXSound = "1100,300,1500,100,1900,200,1500,200";
    ...
    if( !JanHelp.IsEmpty() ){
        ...
        sprintf(bf, "&Ayuda (%s)", JanHelp.c_str());
		pm->Caption = bf;
...
{
Memo2->Lines->Values["qrz"]=qrz;
Memo2->Lines->Values["banda"]=ComboBox1->Text;
...
void __fastcall TMmsstv::UpdateToneFreq(void)
{
	...
	char bf[128];
	sprintf(bf, MsgEng ? "Enviar tono de %uHz" : "%uHzƒ∞›ÇÃëóêM", fq);
	SBTune->Hint = bf;
	SBTune->Caption = fq;
}
//---------------------------------------------------------------------------
void __fastcall TMmsstv::UpdateSystemFont(void)
{
	...
	if( (sys.m_WinFontName != Font->Name)||
		...
		GB1->Font->Style = fsw;
		GroupBox1->Font->Name = sys.m_WinFontName;
		GroupBox1->Font->Charset = sys.m_WinFontCharset;
		GroupBox1->Font->Style = fsw;
	...
	if( Font->Charset != SHIFTJIS_CHARSET ){
		MsgEng = 1;
		KFile->Caption = "&Archivo";
		KView->Caption = "&Ver";
		KOpt->Caption = "&Opciones";
//		KExtCmd->Caption = "Program(&P)";
		KHelp->Caption = "&Ayuda";
// File Nenu for english
		KLogOpen->Caption = "Abrir &Log...";
		KLogFlush->Caption = "Guardar ahora(&F)";
		KFRecTim->Caption = "Grabar sonido con linea de tiempo(&A)";
		KFRec->Caption = "Grabar sonido(&W)...";
		KFPlay->Caption = "&Reproducir sonido desde archivo...";
		KFPlayPos->Caption = "Reproducir posiciÛn";
		KFRecRew->Caption = "Rebobinar";
		KFRecStop->Caption = "Detener grabaciÛn / Play";
		KExit->Caption = "Salir MMSSTV(&X)";

		KEdit->Caption = "&Editar";
		KEC->Caption = "&Copiar ventana historia";
		KEE->Caption = "&Editar imagen TX...";
		KER->Caption = "Copiar ventana &RX";
		KEA->Caption = "&Auto-copiar";
		KEP->Caption = "&Pegar en TX...";
		KES->Caption = "&Pegar y ajustar en TX";
		KEX->Caption = "Pegar a travÈs de Clipper...";
		KEW->Caption = "Ajuste 320x240";
		KEW1->Caption = "Ajuste vertical";
		KEW2->Caption = "Auto-ajuste";
		KEW3->Caption = "No ajustar";
		KEW4->Caption = "Desplazar imagen para el encabezado";

		KVR->Caption = "Mostrar siempre el visor RX";
		KVS->Caption = "Mostrar siempre el visor de sincronizaciÛn";
		KVH->Caption = "Mostrar siempre miniaturas de la historia";
		KVF->Caption = "Mostrar siempre miniaturas de la carpeta de archivos";
		KVC->Caption = "Mostrar siempre los botones de control";
		KVSD->Caption = "DiseÒo";
		KVSD1->Caption = "Estandar";
		KVSD2->Caption = "Vista de stock separada";
		KVSD3->Caption = "Separar todas las vistas";
		KRCD->Caption = "Hijo de una ventana de escritorio";
		KVL->Caption = "Indicador de nivel";
		KVLSG->Caption = "SeÒales de recepciÛn";
		KVLSY->Caption = "Sincronizar seÒales";
		KVSE1->Caption = "Vista del stock";
		KVSE2->Caption = "Vista de ajustes";
		KVSE3->Caption = "Vista de log";

		KS->Caption = "VisualizaciÛn espectral";
		KSFQ->Caption = "Demodulador FM";
		KFFTW->Caption = "Rango de visualizaciÛn espectral";
		KFFTS->Caption = "Espectral trail";
		KFSQ->Caption = "R·pido";
		KFSS->Caption = "Corto";
		KFSL->Caption = "Largo";
		KF->Caption = "Sensibilidad";
		KR->Caption = "Respuesta";
		KRS->Caption = "Lento";
		KRM->Caption = "Medio";
		KRF->Caption = "R·pido";
		KFTD->Caption = "Prioridad de c·lculo";
		KFTD1->Caption = "Bajo";
		KFTD2->Caption = "Medio";
		KFTD3->Caption = "Medio alto";
		KFTD4->Caption = "Alto";
		KFTD5->Caption = "Muy alto";
		KOSC->Caption = "&Osciloscopio...";
		KVOut->Caption = "Nivel de salida de sonido(&V)...";
		KVIn->Caption = "Nivel de &entrada de sonido...";
		KORep->Caption = "Repetidor...";
		KOL->Caption = "ConfiguraciÛn del &Log...";
		KOM->Caption = "Configurar MMSSTV(&O)...";

		KP->Caption = "P&erfiles";
		KPDef->Caption = "Por defecto";
		KPInit->Caption = "Inicializar todos los men˙s por defecto";
		KPA->Caption = "Crear perfil";
		KPD->Caption = "Eliminar perfil";

		KX->Caption = "&Programa";
		KXP->Caption = "MS &Paint";
		KXA->Caption = "Asignar programa";
		KXD->Caption = "Eliminar programa";
		KXR->Caption = "Restaurar";
		KXS->Caption = "&Suspender";
		KXSM->Caption = "Suspender+&Minimizar";

		KRadio->Caption = "Omni-Rig";
		KRadioS->Caption = "Editar men˙";
		KRLoad->Caption = "&Cargar...";
		KRSet->Caption = "&Configurar...";
		KRadioAdd->Caption = "&Agregar man˙...";

		KHlpUp->Caption = "VersiÛn-up...";
		KHlpPad->Caption = "Usar NotePad";
		KHJ->Caption = "JASTA Contest (Agosto)";
		KHN->Caption = "NVCG Contest (Julio)";
		KVer->Caption = "Version(&A)...";

		TabSync->Caption = "Sync";
		TabRX->Caption = "RX";
		TabHist->Caption = "Historia";
		TabTX->Caption = "TX";
		TabTemp->Caption = "Plantilla";
		SBPhase->Caption = "Phase";
		SBSlant->Caption = "Incl.";
		SBSampSet->Caption = "Mem";
		TabS->Tabs->Strings[0] = "S.pix";
		TabS->Tabs->Strings[1] = "Plantillas1";
		TabS->Tabs->Strings[2] = "2";
		TabS->Tabs->Strings[3] = "3";
		TabS->Tabs->Strings[4] = "4";

		CBSTemp->Caption = "Mostrar con plantilla";
		CBSDraft->Caption = "Borrador";

		KRFS->Caption = "ReSync";
		KRC->Caption = "&Copiar";
		KRCC->Caption = "Limpiar";
		KRCS->Caption = "Ajuste de inclinaciÛn de alta precisiÛn";
		KRCR->Caption = "SincronizaciÛn de alta precisiÛn";
		KRSR->Caption = "Volver a la frecuencia de muestreo de configuraciÛn";
		KRI->Caption = "PrecisiÛn de sincronizaciÛn inicial";
		KRI1->Caption = "Normal";
		KRI2->Caption = "Alto";
		KRI3->Caption = "Muy alto";
		KRAS->Caption = "Auto stop";
		KRAR->Caption = "Reinicio autom·tico";
		KRASY->Caption = "ResincronizaciÛn autom·tica";
		KRSW->Caption = "Cambio autom·tico a la ventana RX";
		KRSA->Caption = "Ajuste de inclinaciÛn autom·tica";
		KRH->Caption = "Copiar al historial";
		KRR->Caption = "Cambiar tamaÒo a RX";
		KRRC->Caption = "Cambiar a 320x256";
		KRB->Caption = "Color de fondo...";
		KRCan->Caption = "Cancelar";
		KRD->Caption = "Borrado autom·tico";

		KSP->Caption = "&Pegar";
		KSD->Caption = "&Eliminar";
		KSL->Caption = "&Cargar desde archivo...";
		KSS->Caption = "&Guardar en archivo...";
		KSE->Caption = "&Editar imagen...";
		KSCD->Caption = "&Mover carpeta...";
		KSFJ->Caption = "Usar formato JPEG";
		KSFB->Caption = "Usar formato BMP";
		KSMS->Caption = "Crear imagen de catalogo";
		KSTP->Caption = "PatrÛn de prueba";
		KSAI->Caption = "Ajuste de imagen...";
		KSAM->Caption = "Margen autom·tico";
		KSIS->Caption = "Estirar imagen TX";
		KSRR->Caption = "Cambio autom·tico a la ventana RX";
		KSCan->Caption = KRCan->Caption;
		KSF->Caption = "Cambio de tamaÒo de alta calidad con miniatura";
		KSA->Caption = "Agregar los elementos a la plantilla";

		KTPA->Caption = KSAI->Caption;
		KTPI->Caption = "Pegar imagen";
		KTPB->Caption = "Mapa de bits";
		KTPO->Caption = "SuperposiciÛn";
		KTKA->Caption = "Mantener relaciÛn de aspecto";
		KTOS->Caption = "Cambiar al tamaÒo original";
		KTU->Caption = "&Deshacer";
		KTC->Caption = "&Cortar";
		KTP->Caption = "&Pegar";
		KTD->Caption = "&Eliminar";
		KTO->Caption = "&Objeto";
		KTOI->Caption = "&Insertar...";
		KTCI->Caption = "Insertar elemento personalizado";
		KTOCB->Caption = "Copiar como bitmap";
		KTOB->Caption = "Estirar bitmap";
		KTOR->Caption = "Propiedades";
		KTE->Caption = "Agregar texto a la lista...";
		KTB->Caption = "&Al fondo";
		KTF->Caption = "&Al frente";
		KTTD->Caption = "Mostrar punto transparente";
		KTT->Caption = "&Punto transparente";
		KTAP->Caption = "Punto transparente autom·tico";
		KTTC->Caption = "&Fondo transparente...";
		KTAC->Caption = "Color de fondo";
		KTLI->Caption = "ArtÌculos";
		KTSA->Caption = "Todos";
		KTSI->Caption = "ArtÌculos seleccionados";
		KTA->Caption = "&Eliminar todo";
		KTDF->Caption = "Muestras";
		KTMD->Caption = "Dibujar texto ficticio de macro";
		KTBT->Caption = "Mostrar imagen TX";
		KTSZ->Caption = "TamaÒo";
		KTSZT->Caption = "Adaptar con imagen TX";
		KTSD->Caption = "Mostrar tamaÒo";
		KTX->Caption = "Opciones";
		KTFS->Caption = "Deshabilitar suavizado de fuente";
		KTH->Caption = "Utilizar cÛdigo r·pido";
		KTCan->Caption = KRCan->Caption;

		KHCD->Caption = "&Mover carpeta...";
		KHAP->Caption = KTPA->Caption;
		KHS->Caption = "&Guardar a archivo...";
		KHT->Caption = "Guardar a archivo con &linea de tiempo...";
		KHD->Caption = "&Eliminar";
		KHDA->Caption = "Eliminar todo";
		KHFJ->Caption = KSFJ->Caption;
		KHFB->Caption = KSFB->Caption;
		KHAC->Caption = "Copia autom·tica a otra carpeta";
		KHACD->Caption = "Asignar otra carpeta...";
		KHACT->Caption = "Escribir marca de tiempo en la imagen copiada";
		KHACJ->Caption = "Copiar como formato JPEG";
		KHDS->Caption = "Arrastrar y soltar";
		KHTB->Caption = "Mostrar barra de tÌtulo";
		KHVS->Caption = "TamaÒo de miniatura";
		KHCan->Caption = KRCan->Caption;

		KHWeb->Caption = "MMSSTV Web";
		KHRef->Caption = "MMSSTV Reflector";

		KCWM->Caption = "Editar man˙";
		KCWCancel->Caption = "Cancelar";
		KCWS->Caption = "Velocidad";
		KCWADD->Caption = "Nuevo men˙...";

		SBPL->Hint = "Desplazar a la izquierda";
		SBPR->Hint = "Desplazar a la derecha";
		SBPhase->Hint = "Ajuste de sincronizaciÛn manual";
		SBSlant->Hint = "Ajuste de inclinaciÛn manual";
		CBASave->Caption = "Historial auto";
		CBASave->Hint = "Guardar autom·ticamente en el historial";
		SBSampSet->Hint = "Frecuencia de muestreo de memoria";
		SBTO->Hint = "Desplazamiento TX";
		SBLK->Caption = "Bloquear";
		SBLK->Hint = "Bloquear";
		SBWHist->Hint = "Guardar en el historial";
		SBRView->Hint = "Visor de imagenes";
		SBCopy->Hint = "Copiar";
		SBHView->Hint = "Visor de imagenes";

		SBCW->Hint = "Salida CW (Haga clic derecho para el men˙ CW)";
		SBUseTemp->Hint = "Usar plantilla";
		SBUS240->Hint = "Cambiar imagen y plantilla para modo de 240 lÌneas";
		SBDS240->Hint = "Desplazar imagen para la barra de color del encabezado";
		SBUA240->Hint = "Imagen de ajuste y plantilla para el modo de 240 lÌneas";
		SBDA240->Hint = "Imagen de ajuste para la barra de color del encabezado";
		SBPaste->Hint = "Pegar";
		SBTView->Hint = "Visor de imagen";
		SBDSel->Hint = "Seleccionar elemento";
		SBDline->Hint = "LÌnea";
		SBDBox->Hint = "Cuadro";
		SBDBoxS->Hint = "Cuadro lleno";
		SBDText->Hint = "Texto";
		SBDPic->Hint = "Imagen";
		SBDHead->Hint = "Barra de colores";
		SBDList->Hint = "Texto definido";
		SBDDel->Hint = "Cortar";
		SBDPaste->Hint = "Pegar";
		SBDCol->Hint = "Color";
		SBDLineset->Hint = "Estilo de linea";
		SBDFont->Hint = "Fuente";

		SBQSO->Hint = "Iniciar/Finalizar QSO";
		SBData->Hint = "Datos";
		SBFind->Hint = "Buscar Indicativo";
		SBInit->Hint = "Limpiar datos";
		SBList->Hint = "Abrir MMLog";
		SBULog->Hint = "Actualizar plantilla";

		SBHistFil->Hint = "Ajustar imagen";
		SBRXID->Hint = "Decodificar FSKID";
		SBTXID->Hint = "Codificar FSKID";
		SBAdj->Hint = "Ajuste de inclinaciÛn de alta precisiÛn (clic derecho para el ajuste de sincronizaciÛn)";
	}
	else {
		MsgEng = 0;
	}
...
void __fastcall TMmsstv::ReadRegister(void)
{
	...
	sys.m_WaterViewLeft = pIniFile->ReadInteger("Window", "WaterViewLeft", GroupBox1->Left);
	sys.m_WaterViewWidth = pIniFile->ReadInteger("Window", "WaterViewWidth", GroupBox1->Width);
	...
	sys.m_LogViewTop = pIniFile->ReadInteger("Window", "LogViewTop", GroupBox1->Top);
	sys.m_LogViewLeft = pIniFile->ReadInteger("Window", "LogViewLeft", GroupBox1->Left);
	sys.m_LogViewWidth = pIniFile->ReadInteger("Window", "LogViewWidth", GroupBox1->Width);
	sys.m_LogViewHeight = pIniFile->ReadInteger("Window", "LogViewHeight", GroupBox1->Height);
...
// ÉäÉsÅ[É^
		...
		ErrorMB(MsgEng?"No se puede actualizar MMSSTV.INI":"MMSSTV.INIÇçXêVÇ≈Ç´Ç‹ÇπÇÒ.");
	}
...
void __fastcall TMmsstv::UpdateTitle(void)
{
	...
	if( SampType ){
		sprintf(bbf, " [basado en %.lfHz]", SampBase);
		strcat(bf, bbf);
	}
	if( sys.m_BitPixel < 24 ){
		sprintf(bbf, " [%u colores]", int(pow(2, sys.m_BitPixel)));
		strcat(bf, bbf);
	}
	if( sys.m_bCQ100 ){
		strcat(bf, " por Internet");
    }
	Caption = bf;
	UpdateCallsign();
}
...
void __fastcall TMmsstv::FormPaint(TObject *Sender)
{
	...
	if( m_InitFirst ){
		m_InitFirst = 0;
		int f1stInst = FALSE;
		if( (sys.m_Call == "NONE") || sys.m_Call.IsEmpty() ){
            f1stInst = TRUE;
			AnsiString as = "";
			if( sys.m_Call != "NONE" ){
				as = sys.m_Call;
			}
			if( InputMB("MMSSTV", MsgEng ? "Introduce tu indicativo":"Ç†Ç»ÇΩÇÃÉRÅ[ÉãÉTÉCÉìÇì¸óÕÇµÇƒâ∫Ç≥Ç¢.", as) == TRUE ){
				if( !as.IsEmpty() ){
					char bf[MLCALL+1];
...
void __fastcall TMmsstv::EntryAlign(void)
{
	...
	AlignFix.EntryControl(GroupBox1, &rc, GroupBox1->Font);
	...
	AlignLog.EntryControl(Label3, GBLog, NULL);
	...
	AlignLog.EntryControl(ComboBox1, GBLog, NULL);
	...
	AlignLog.EntryControl(Label1, GBLog, NULL);
	AlignGroup.EntryControl(SpeedButton1, GroupBox1, NULL);
	AlignGroup.EntryControl(SpeedButton10, GroupBox1, NULL);
	AlignGroup.EntryControl(SpeedButton2, GroupBox1, NULL);
	AlignGroup.EntryControl(SpeedButton3, GroupBox1, NULL);
	AlignGroup.EntryControl(SpeedButton4, GroupBox1, NULL);
	AlignGroup.EntryControl(SpeedButton5, GroupBox1, NULL);
	AlignGroup.EntryControl(SpeedButton6, GroupBox1, NULL);
	AlignGroup.EntryControl(SpeedButton7, GroupBox1, NULL);
	AlignGroup.EntryControl(SpeedButton8, GroupBox1, NULL);
	AlignGroup.EntryControl(SpeedButton9, GroupBox1, NULL);
...
void __fastcall TMmsstv::FormResize(TObject *Sender)
{
	...
	AlignGroup.NewAlign(GroupBox1); MultProc();
  ...
  rc.Top = 0;
	...
	if( pSound->m_Err || pDem->m_OverFlow ){
		if( pSound->m_Err || !SBTX->Down ){
			...
			tp->Font->Height = -16;
			strcpy(bf, pSound->m_Err ? "PÈrdida de sonido" : "Overflow");
			x = tp->TextWidth(bf);
...
//---------------------------------------------------------------------------
// ÉXÉåÉbÉhì‡Ç≈ÉRÅ[ÉãÇµÇƒÇÕÇ¢ÇØÇ»Ç¢
...
//                ::SetWindowPos(pRxView->Handle, HWND_TOPMOST, 0, 0, 0, 0, SWP_NOMOVE|SWP_NOSIZE	);
		...
		if( pDem->m_fskrec ){
			pDem->m_fskrec = 0;
			if( pDem->m_Sync && ((pCtrBtn == NULL) || (!pCtrBtn->SBLock->Down)) ){     // ã≠êßí‚é~
			...
			if( strcmp(sys.m_Call.c_str(), pDem->m_fskcall) ){
				...
				if( !SBTX->Down && (!SBQSO->Down || HisCall->Text.IsEmpty()) ){
					if( strcmp(AnsiString(HisCall->Text).c_str(), pDem->m_fskcall) ){	//ja7ude 0428
                        ...
                        qrzcom *mythread = new qrzcom(false);
						FindCall();
	                    HisCallChange(NULL);
					}
				}
...
{
	if( (sys.m_AutoStop || sys.m_AutoSync || KRSA->Checked) && (SSTVSET.m_Mode != smAVT) ){   // é©ìÆí‚é~ÇÃÉ`ÉFÉbÉN
...
void __fastcall TMmsstv::DrawSSTVNormal(short *ip, short *sp)
{
	...
	prgs = int(double(n)/SSTVSET.m_TW);
	ProgressBar2->Max = SSTVSET.m_L;
	ProgressBar4->Max = SSTVSET.m_L;
	ProgressBar2->Position = prgs;
	Label5->Caption = IntToStr(porcentaje(SSTVSET.m_L, prgs))+"%";
	Label6->Caption = IntToStr(porcentaje(SSTVSET.m_L, prgs))+"%";
	ProgressBar4->Position = prgs;
...
{
	InfoMB( MsgEng ?
		"MMSSTV perdiÛ algo de sonido durante la recepciÛn por la sobrecarga de la CPU.\r\n\r\n"
		"Aumente FIFO del sonido en la ventana de configuraciÛn de MMSSTV." :
		"éÛêMíÜÇ…CPUâﬂïââ◊Ç…ÇÊÇËÉTÉEÉìÉhÇàÍïîåáóéÇµÇ‹ÇµÇΩ.\r\n\r\n"
		"ê›íËâÊñ ÇÃÅuÇªÇÃëºÅvÉyÅ[ÉWÇÃÉTÉEÉìÉhÉJÅ[ÉhÇÃFIFOÇëùÇ‚ÇµÇƒâ∫Ç≥Ç¢"
	);
}
void __fastcall TMmsstv::InfoTxLost(void)
{
	InfoMB( MsgEng ?
		"MMSSTV perdiÛ algo de sonido durante la recepciÛn por la sobrecarga de la CPU.\r\n\r\n"
		"Aumente FIFO del sonido en la ventana de configuraciÛn de MMSSTV." :
		"ëóêMíÜÇ…CPUâﬂïââ◊Ç…ÇÊÇËÉTÉEÉìÉhÇàÍïîåáóéÇµÇ‹ÇµÇΩ.\r\n\r\n"
		"ê›íËâÊñ ÇÃÅuÇªÇÃëºÅvÉyÅ[ÉWÇÃÉTÉEÉìÉhÉJÅ[ÉhÇÃFIFOÇëùÇ‚ÇµÇƒâ∫Ç≥Ç¢"
	);
}
//---------------------------------------------------------------------------
...
void __fastcall TMmsstv::PBoxRXPaint(TObject *Sender)
{
	...
	DrawBitmap(PBoxRX, pBitmapRX, ProgressBar2);
...
void __fastcall TMmsstv::KFRecClick(TObject *Sender)
{
	if( pSound == NULL ) return;
	if( MsgEng ){
		SaveDialog->Title = "Grabar archivo de sonido MMSSTV";
		SaveDialog->Filter = "MMSSTV Sound Files(*.mmv)|*.mmv|";
	}
	else {
		SaveDialog->Title = "MMSSTVÉTÉEÉìÉhÉtÉ@ÉCÉãÇÃçÏê¨";
		SaveDialog->Filter = "MMSSTVÉTÉEÉìÉhÉtÉ@ÉCÉã(*.mmv)|*.mmv|";
	}
...
void __fastcall TMmsstv::KFPlayClick(TObject *Sender)
{
	...
	if( MsgEng ){
		OpenDialog->Title = "Reproducir archivo de sonido MMSSTV";
		OpenDialog->Filter = "MMSSTV Sound Files(*.mmv)|*.mmv|";
	}
	else {
		OpenDialog->Title = "MMSSTVÉTÉEÉìÉhÉtÉ@ÉCÉãÇÃçƒê∂";
		OpenDialog->Filter = "MMSSTVÉTÉEÉìÉhÉtÉ@ÉCÉã(*.mmv)|*.mmv|";
	}
	OpenDialog->FileName = "";
...
void __fastcall TMmsstv::CorrectSlant(void)
{
	...
	if( ((dp->m_StgBuf != NULL) || WaveStg.IsOpen()) && (dp->m_wStgLine >= 16) && (SSTVSET.m_Mode != smAVT) ){
		...
		int LW = SSTVSET.m_TW * 0.1;       // 10%ÇÃóhÇÍÇãñóe
		int z;
		for( z = 0; z < 5; z++ ){
			// äÓèÄà íuÇíTÇ∑
			WaveStg.RInit();
			...
			for( i = 0; i < dp->m_wStgLine; i++ ){
				...
				for( j = 0; j < SSTVSET.m_WD; j++, sp++, base++ ){
					...
					if( yy != y ){
						if( bpos < 0 ){
							if( ps >= (SSTVSET.m_TW/4) ){         // ç∂ï˚å¸Ç÷ÇÃé¸ÇËÇ±Ç›
								ps -= SSTVSET.m_TW;
							}
						...
						else if( bpos >= SSTVSET.m_TW ){        // âEï˚å¸Ç÷ÇÃé¸ÇËÇ±Ç›
						...
						else if( bpos >= (SSTVSET.m_TW*3/4) ){  // âEë§
							if( (ps < SSTVSET.m_TW/4) ){
								ps += SSTVSET.m_TW;
							}
						}
						else if( bpos <= (SSTVSET.m_TW/4) ){    // ç∂ë§
							if( ps >= (SSTVSET.m_TW*3/4) ){
								ps -= SSTVSET.m_TW;
							}
						}
...
int __fastcall TMmsstv::AdjustSyncPos(int n)
{
		...
		switch(SSTVSET.m_Mode){
			...
			case smSCTDX:
				...
				break;
			case smMRT1:        // å¥àˆïsñæÇÃÉ^ÉCÉ~ÉìÉOÉYÉåÇï‚ê≥
			case smSC2_180:
			case smSC2_120:
			case smP3:
			case smP5:
...
{
	if( pSound == NULL ) return;
	if( m_ReqSampChg ){             // ÉTÉìÉvÉäÉìÉOé¸îgêîïœçXóvãÅíÜ
		m_ReqSampChg = 0;
		RedrawSampFreq(FALSE);
		return;
	}
...
{
#if 1
	// TapêîÇëùÇ‚ÇµÇΩèÍçáÇÃèàóù
    // 2003/11/28Ç…é¿å±ÇµÅAåãâ TapêîÇÕè]óàí ÇËÇÃÇSÇ…ñﬂÇµÇΩ
    // ÇÃÇ≈Ç±ÇÃèàóùÇÉLÉÉÉìÉZÉãÇµÇΩ
    int d = GetLMSDelay();
...
{
	UDHist->Max = RxHist.m_Head.m_Cnt-1;
	if( m_DisEvent ) return;
	if(Button==btNext){
		UDHist->Position=UDHist->Position+Increment;
		if( UDHist->Position < RxHist.m_Head.m_Cnt){
		UpdateHist();
		}else{
			 UDHist->Position = RxHist.m_Head.m_Cnt-1;
			 UpdateHist();
		}
	}
	else if(Button==btPrev){
		UDHist->Position=UDHist->Position-Increment;
		if( UDHist->Position >=0 ){
		 UpdateHist();
		 }else{
			 UDHist->Position = 0;
			 UpdateHist();
...
{
	if( UDHist->Position < RxHist.m_Head.m_Cnt ){
		if( YesNoMB( MsgEng ? "Borrar·s la imagen, øest·s seguro?" : "ÇŸÇÒÇ‹Ç…è¡ÇµÇ‹Ç∑Ç©ÅH" ) == IDYES ){
...
void __fastcall TMmsstv::KHDAClick(TObject *Sender)
{
	if( RxHist.m_Head.m_Cnt ){
		if( YesNoMB( MsgEng ? "Borrar·s todas las imagenes, øest·s seguro?" : "ëSÇƒÇÃóöóÇçÌèúÇµÇ‹Ç∑." ) == IDYES ){
			RxHist.DeleteAll();
			UDHist->Position = 0;
			UpdateHist();
...
{
	DrawBitmap(PBoxHist, pBitmapHist, ProgressBar3);
...
void __fastcall TMmsstv::PBoxTXPaint(TObject *Sender)
{
	...
	DrawBitmap(PBoxTX, pBitmapTX, ProgressBar3);
	m_SendY = -1;
	if( sys.m_ShowSizeTX ){
	...
	if( IsSR() ){
		if( mp->m_wLine >= SSTVSET.m_TL ){
			if( mp->m_wLine == SSTVSET.m_TL ){
				...
				int lost = mp->m_Lost;
//                ::Sleep( (pSound->m_BuffSize * pSound->m_FifoSize * 1000) / SSTVSET.m_TxSampFreq ); // ÉoÉbÉtÉ@ëóêMäÆóπë“Çø
				ToRX();
				if( lost ) InfoTxLost();
				return;
			}
...
//        int l = (mp->m_rCnt - mp->m_sCnt - ) / SSTVSET.m_TWD;
		...
		if( l < 0 ) l = 0;
		DrawTXCursor(l);
		ProgressBar1->Position = porcentaje(PBoxTX->Height, l);
		Label4->Caption = IntToStr(porcentaje(PBoxTX->Height, l))+"%";
	}
}
//---------------------------------------------------------------------------
...
void __fastcall TMmsstv::ToTX(void)
{
	...
	if( SBUseTemp->Down && DrawMain.m_Cnt && (pBitmapTXM->Width != DrawMain.m_SX) && (!rep) ){
		SBTX->Down = FALSE;
		if(omni=="1")OmniRig->Rig1->Tx = RigParamX::PM_RX;
		if(
			YesNoMB( MsgEng ?
			"La calidad de la imagen de la plantilla no es buena, porque el tamaÒo de la plantilla es diferente de la imagen TX..\r\n\r\nøQuiÈres continuar?" :
			"ÉeÉìÉvÉåÅ[ÉgÇÃÉTÉCÉYÇ™ëóêMâÊëúÇ∆àŸÇ»ÇÈÇΩÇﬂÉeÉìÉvÉåÅ[ÉgÇÃâÊéøÇÕó«Ç≠Ç†ÇËÇ‹ÇπÇÒ.\r\n\r\nÇ±ÇÃÇ‹Ç‹ëóêMÇµÇ‹Ç∑Ç©ÅH" ) == IDNO
		){
			return;
		}
		SBTX->Down = TRUE;
		if(omni=="1")OmniRig->Rig1->Tx = RigParamX::PM_TX;
	}
	...
	OutHEAD();
	// VIS
	if( SSTVSET.m_fTxNarrow ){			// ã∑ë—àÊÉÇÅ[ÉhÇÃëóêM
		mp->Write(1900, 300);
		mp->Write(FSKSPACE, FSKGARD);
		mp->Write(1900, FSKINTVAL);
...
//		mp->Write(FSKSPACE, FSKGARD);
		...
		if( SSTVSET.m_TxMode == smAVT ){       // ìØä˙êMçÜ
			int sd = 0x5fa0;
			for( i = 0; i < 32; i++ ){
...
void __fastcall TMmsstv::SBTXClick(TObject *Sender)
{
	...
	if( SBTX->Down ){
		//if(omni=="1")OmniRig->Rig1->Tx = RigParamX::PM_TX;
		//ProgressBar1->Max = PBoxTX->Height;
		ToTX();
	}
	else {
		//if(omni=="1")OmniRig->Rig1->Tx = RigParamX::PM_RX;
...
_n1:;
	...
	if( SBUseTemp->Down || (m_MainPage == pgTemp) ){
		...
		if( rep && sys.m_RepBottomAdj ){    // êLèkÇ»Çµ
...
void __fastcall TMmsstv::InfoSampFreq(void)
{
	if( fabs(sys.m_SampFreq - SampFreq) >= 50.0 ){
		...
		InfoMB(
			MsgEng ?
			"Reinicie MMSSTV para la nueva frecuencia de muestreo.\r\n(No tiene que reiniciar su PC)\r\n\r\n"
			"Ahora MMSSTV se ejecuta en %.2lfHz, y se le solicitÛ que cambiara a %.2lfHz."
			"  Esta diferencia es demasiado grande para el DSP de MMSSTV." :
			"êVÇµÇ¢ÉTÉìÉvÉäÉìÉOÉåÅ[ÉgÇìKópÇµÇ‹Ç∑ÅBMMSSTVÇçƒãNìÆÇµÇƒâ∫Ç≥Ç¢\r\n"
			"(ÉpÉ\ÉRÉìÇçƒãNìÆÇ∑ÇÈïKóvÇÕÇ†ÇËÇ‹ÇπÇÒ)\r\n\r\n"
			"åªç›MMSSTVÇÕ%.2lfHzÇ≈ìÆçÏÇµÇƒÇ®ÇËÅA%.2lfHzÇ÷ÇÃïœçXÇóvêøÇ≥ÇÍÇƒÇ¢Ç‹Ç∑ÅB"
			"  çƒãNìÆÇµÇ»Ç¢Ç∆ê≥èÌÇ»ìÆçÏÇà€éùÇ∑ÇÈÇ±Ç∆Ç™Ç≈Ç´Ç‹ÇπÇÒÅB"
	...
	if( strcmpi(sys.m_TxRxName.c_str(), "NONE") ){
		...
		if( !pComm->IsOpen() ){
			if( sys.m_TxRxLock ){
				delete pComm;
				pComm = NULL;
				ErrorMB( "No se pudo abrir '%s' para los controles PTT", sys.m_TxRxName.c_str());
			}
		}
		if( pComm != NULL ){
...
//        if( !sys.m_TxRxLock ) pComm->Close();
	...
	if( strcmp(RADIO.StrPort, "NONE" ) ){
		...
		if( pRadio->m_CreateON != TRUE ){
			delete pRadio;
			pRadio = NULL;
			ErrorMB( MsgEng ? "No se pudo abrir '%s' para comandos de radio":"RadioÉRÉ}ÉìÉhÉ|Å[Ég'%s'Ç™ÉIÅ[ÉvÉìÇ≈Ç´Ç‹ÇπÇÒ.", RADIO.StrPort);
		}
		else {
...
void __fastcall TMmsstv::UpdateLogData(void)
{
	...
	//Log.SetFreq(&Log.m_sd, AnsiString(LogFreq->Text).c_str());
	if (ComboBox1->Text == "160M") {
		f_rec = "1.8";
	} else if (ComboBox1->Text == "80M") {
		f_rec = "3.5";
	} else if (ComboBox1->Text == "60M") {
		f_rec = " ";
	} else if (ComboBox1->Text == "40M") {
		f_rec = "7";
	} else if (ComboBox1->Text == "30M") {
		f_rec = "10";
	} else if (ComboBox1->Text == "20M") {
		f_rec = "14";
	} else if (ComboBox1->Text == "17M") {
		f_rec = "18";
	} else if (ComboBox1->Text == "15M") {
		f_rec = "21";
	} else if (ComboBox1->Text == "12M") {
		f_rec = "24";
	} else if (ComboBox1->Text == "11M") {
		f_rec = "";
	} else if (ComboBox1->Text == "10M") {
		f_rec = "28";
	} else if (ComboBox1->Text == "6M") {
		f_rec = "50";
	} else if (ComboBox1->Text == "2M") {
		f_rec = "144";
	} else if (ComboBox1->Text == "70CM") {
		f_rec = "430";
	}
	Log.SetFreq(&Log.m_sd, AnsiString(f_rec).c_str());	//ja7ude 0428
	//"1.8,3.5,3.8,7,7.033,10,14,14.230,18,21,21.340,24,28,28.680,50,144,430,1200,2400"
}
//---------------------------------------------------------------------------
...
void __fastcall TMmsstv::FindCall(void)
{
	...
	if( Log.FindSet(&Log.m_Find, Log.m_sd.call) ){  // å©Ç¬Ç©Ç¡ÇΩéû
		MultProc();
		Log.SetFreq(&Log.m_sd, AnsiString(ComboBox1->Text).c_str());	//ja7ude 0428
		SDMMLOG	sd;
	...
	if( !SBQSO->Down ){
		...
		LogLink.SetFreq(AnsiString(ComboBox1->Text).c_str());	//ja7ude 0428
...
void __fastcall TMmsstv::SBQSOClick(TObject *Sender)
{
	...
	int i;
	AnsiString parametros, send, band, call, freq, mode, qso_date, time_on, rst_sent, rst_rcvd, tx_pwr, comment;
	UpdateLogData();
	SYSTEMTIME	now;
	GetUTC(&now);
	...
	int Day = now.wDay;
	int Hour = now.wHour;
	AnsiString h_ora = IntToStr(now.wHour);
	String m_inutos = IntToStr(now.wMinute);
	String s_egundos = IntToStr(now.wSecond);
	String m_es = IntToStr(Month);
	String d_ia = IntToStr(Day);
	UTCtoJST(Year, Month, Day, Hour);
	String fecha, dia, mes, his, my, frec;
	if (m_es.ToInt()<10) {
	   m_es = "0"+m_es;
	}
	if (d_ia.ToInt()<10) {
	...
	if (m_inutos.ToInt()<10) {
	   m_inutos = "0"+m_inutos;
	}
	if (s_egundos.ToInt()<10) {
	   s_egundos = "0"+s_egundos;
	}
	fecha = "20"+IntToStr(Year)+m_es+d_ia;
	if(api==1) {
	band = adif("BAND", ComboBox1->Text);
	call = adif("CALL", HisCall->Text);
	if (omni == "1") {
		if (Label1->Caption != " FUERA DE BANDA") {
			freq = Frecuencia_Sola(OmniRig->Rig1->Freq);
			freq = adif("FREQ", freq);
		}
	}
	mode = adif("MODE", "SSTV");
	qso_date = adif("QSO_DATE", fecha);
	time_on = adif("TIME_ON", h_ora+m_inutos+s_egundos);
	rst_sent = adif("RST_SENT", HisRST->Text);
	if (MyRST->Text.Length()>0) {
	   rst_rcvd = adif("RST_RCVD", MyRST->Text);
	}else rst_rcvd = adif("RST_RCVD", "595");
	//rst_rcvd = adif("RST_RCVD", MyRST->Text);
	comment = adif("COMMENT", EditNote->Text);
	tx_pwr = adif("TX_PWR", "");
	send = band+call+freq+mode+qso_date+time_on+rst_sent+rst_rcvd+tx_pwr+comment+"<EOR>";
	parametros = adif("parameters", send);
	ClientSocket1->Active = true;
	ClientSocket1->Socket->SendText("<command:3>Log "+parametros);
...
//	Log.m_sd.cq = m_Running ? 'A' : 'C';
	if( SBQSO->Down ){		// Start QSO
		...
		Memo1->Lines->Strings[0] = fecha;
		Memo1->Lines->Strings[1] = UpperCase(HisCall->Text);
		Memo1->Lines->Strings[2] = UpperCase(HisName->Text);
		Memo1->Lines->Strings[3] = UpperCase(HisQTH->Text);
		Memo1->Lines->Strings[4] = UpperCase(EditNote->Text);
		Memo1->Lines->Strings[5] = UpperCase(EditQSL->Text);
		switch (HisRST->ItemIndex)
			{
			case 0 : his = "595";break;
			case 1 : his = "575";break;
			case 2 : his = "555";break;
			case 3 : his = "594";break;
			case 4 : his = "574";break;
			case 5 : his = "554";break;
			case 6 : his = "444";break;
			case 7 : his = "333";break;
			default : his = "595";break;
			}
		switch (MyRST->ItemIndex)
			{
			case 0 : my = "595";break;
			case 1 : my = "575";break;
			case 2 : my = "555";break;
			...
			case 6 : my = "444";break;
			case 7 : my = "333";break;
			default : my = "595";break;
			}

		Memo1->Lines->Strings[6] = his;
		Memo1->Lines->Strings[7] = my;
		Memo1->Lines->Strings[8] = LogFreq->Text;
		Memo1->Lines->Strings[9] = "SSTV";

	   //Memo1->Lines->SaveToFile("LogparaGRG.txt", TEncoding::UTF8);
...
void __fastcall TMmsstv::KLogOpenClick(TObject *Sender)
{
	...
	OpenDialog->Options >> ofFileMustExist;
	if( MsgEng ){
		OpenDialog->Title = "Abrir archivo LogData";
	  ...
	  TShiftState Shift, int X, int Y)
{
	...
	if( sc1 == sc2 ){			// Left button
		AdjustTempView(1);
		if( pDraw != NULL ){                // êVãKçÏê¨íÜ
			pDraw->Making(X, Y);
		}
		else if( DrawMain.pSel != NULL ){   // à⁄ìÆ/ÉTÉCÉYïœçXíÜ
			if( !m_TempTimer || (ABS(X-m_TempX)+ABS(Y-m_TempY)) > 5 ){
				m_TempTimer =0;
				DrawMain.pSel->Moving(X, Y);
			}
		}
		else if( !PBoxTemp->Dragging() ){   // ÉhÉâÉbÉO
			//PBoxTemp->BeginDrag(TRUE);
...
//---------------------------------------------------------------------------
// ÉeÉìÉvÉåÅ[Ég ìßâﬂÉ|ÉCÉìÉgÇÃéwíË
...
//    DrawMain.UpdatePic(pBitmapHistF);
	...
	if( KTBT->Checked && !SBTX->Down ){
		...
		MakeTxBitmap(0);
		m_TxClipType = sv;
		DrawBitmap(PBoxTemp, pBitmapTX, ProgressBar3);
		if( DrawMain.pSel != NULL ){
			AdjustTempView(1);
		...
		DrawBitmap(PBoxTemp, pBitmapTemp, ProgressBar3);
...
//                DrawTemp.UpdatePic(pBitmapHistF);
		...
		//Graphics::TBitmap *pBitmapSV = CreateBitmap(512, 400, -1);
		MultProc();
...
int __fastcall TMmsstv::LoadBitmapMenu(Graphics::TBitmap *pBitmap, int rs)
{
	...
	OpenDialog->Options << ofFileMustExist;
	if( MsgEng ){
		OpenDialog->Title = "Cargar imagen del archivo";
	}
	else {
		OpenDialog->Title = "âÊëúÇÃÉçÅ[Éh";
	}
...
int __fastcall TMmsstv::LoadTemplateMenu(CDrawGroup *pItem, int isw)
{
	...
	OpenDialog->Options << ofFileMustExist;
	if( MsgEng ){
		OpenDialog->Title = "Cargar plantilla desde archivo";
	}
	else {
		OpenDialog->Title = "ÉeÉìÉvÉåÅ[ÉgÉtÉ@ÉCÉãÇÃÉçÅ[Éh";
	}
	if( isw ){
		char bf[256];
...
int __fastcall TMmsstv::SaveTemplateMenu(CDrawGroup *pItem, LPCSTR pName, LPCSTR pFolder, int isw)
{
	...
	if( MsgEng ){
		...
		SaveDialog->Title = "ÉeÉìÉvÉåÅ[ÉgÇÉZÅ[Éu";
	}
	if( isw ){
...
void __fastcall TMmsstv::PopupSPopup(TObject *Sender)
{
	...
	else if( TabS->TabIndex ){   // ÉeÉìÉvÉåÅ[Ég
		KSC->Enabled = FALSE;
		KSP->Enabled = FALSE;
		KSFJ->Enabled = FALSE;
		KSFB->Enabled = FALSE;
...
void __fastcall TMmsstv::PopupTPopup(TObject *Sender)
{
	...
	if( cmd != CM_PIC ){
		KTFX->Caption = MsgEng ? "Convertir imagen" : "âÊëúÉAÉCÉeÉÄÇ…ïœä∑";
	}
	else {
		KTFX->Caption = MsgEng ? "Convertir a bitmap" : "ÉrÉbÉgÉ}ÉbÉvÇ…ïœä∑";
	}
		...
		if( (n = Cty.GetNoP(p))!=0 ){
			...
			if( cp->TD != NULL ){
				...
				if( tim ){
					tim /= WORD(30);
					if( tim < 12*60 ){
						strcpy(t, type ? "GM" : "Buenos dÌas");
					}
					else if( tim < 18*60 ){
						strcpy(t, type ? "GA" : "Buenas tardes");
					}
					else {
						strcpy(t, type ? "GE" : "Buenas noches");
					}
				}
			}
		}
		...
		if( *p == '%' ){
			...
			switch(*p){
				...
				case 'u':
					...
					break;
				case 'v':
					strcpy(t, VERID VERBETA);
					break;
				case 'V':
					strcpy(t, VERBETA);
					break;
				case 'B':
					strcpy(t, (f && LogFreq->Text.IsEmpty()) ? "Freq" : AnsiString(ComboBox1->Text).c_str());	//ja7ude 0521
					break;
				case 'b':
					strcpy(t, (f && LogFreq->Text.IsEmpty()) ? "Band" : AnsiString(ComboBox1->Text).c_str());
					break;
				case 'o':
					{
						AnsiString ws;
...
//                            Graphics::TBitmap *pBitmap = LoadBitmapS(m_DragNo);
						...
						ErrorMB("'%s' no es soportado.", fname);
					}
				}
			}
		}
	...
	else if( Msg.hwnd == PanelTX->Handle ){     // ëóêMâÊñ Ç÷ÇÃÉhÉçÉbÉv
		if( ::DragQueryFile(hDrop, 0, fname, sizeof(fname)) ){
			::SetForegroundWindow(Handle);
			LPCSTR pExt = GetEXT(fname);
			if( IsPic(pExt) ){
...
//                ErrorMB("'%s' is not supported.", fname);
	...
	else if( Msg.hwnd == PanelTemp->Handle ){       // ÉeÉìÉvÉåÅ[ÉgÇ÷ÇÃÉhÉçÉbÉv
_tdrop:;
		if( ::DragQueryPoint(hDrop, &N) ){
			if( ::DragQueryFile(hDrop, 0, fname, sizeof(fname)) ){
				::SetForegroundWindow(Handle);
...
{
	if( MsgEng ){
		SaveDialog->Title = "Mover carpeta de archivos de historial";
	}
	else {
		SaveDialog->Title = "óöóÉtÉ@ÉCÉãÇÃÉtÉHÉãÉ_ÇïœçX";
	}
	SaveDialog->Filter = "";
	...
	if( SaveDialog->Execute() == TRUE ){
		...
		if( strcmp(bf, HistDir) ){
			if( YesNoMB( "%s >>> %s, °est·s seguro?", HistDir, bf ) == IDYES ){
				RxHist.MoveDir(bf);
			}
		}
	}
...
{
	if( MsgEng ){
		SaveDialog->Title = "Mover carpeta de archivos de stock";
	}
	else {
		SaveDialog->Title = "ÉXÉgÉbÉNÉtÉ@ÉCÉãÇÃÉtÉHÉãÉ_ÇïœçX";
	}
	SaveDialog->Filter = "";
	...
	if( SaveDialog->Execute() == TRUE ){
		...
		if( strcmp(bf, StockDir) ){
			if( YesNoMB( "%s >>> %s, øest·s seguro?", StockDir, bf ) == IDYES ){
				MoveStockDir(bf);
			}
		}
	}
	  ...
	  TMouseButton Button, TShiftState Shift, int X, int Y)
{
	if( Button == mbRight ){
		...
		if( InputMB("MMSSTV", MsgEng?"Introduce frecuencia del tono (Hz)":"ÉgÅ[Éìé¸îgêî(Hz)Çì¸óÕ", as) == TRUE ){
			int d;
			sscanf(as.c_str(), "%u", &d);
			if( (d >= 100) && (d <= 3000) ){
				pMod->m_TuneFreq = d;
...
void __fastcall TMmsstv::KHFJClick(TObject *Sender)
{
	...
	int r = IDYES;
	if( RxHist.m_Head.m_Cnt ){
		r = YesNoMB( MsgEng ? "MMSSTV convertir· el formato en todos los archivos que ya estaban almacenados.\r\n\rEste procesamiento puede llevar mucho tiempo, øest·s seguro?" : "ä˘ë∂ÇÃÉtÉ@ÉCÉãÇÃÉtÉHÅ[É}ÉbÉgÇïœçXÇµÇ‹Ç∑.\r\n\r\nÇ±ÇÃèàóùÇÕéûä‘Ç™Ç©Ç©ÇÈâ¬î\ê´Ç™Ç†ÇËÇ‹Ç∑. ÇŸÇÒÇ‹Ç…ïœä∑ÇµÇ‹Ç∑Ç©ÅH" );
	}
	if( r == IDYES ){
...
void __fastcall TMmsstv::KSFJClick(TObject *Sender)
{
	...
	int r = YesNoMB( MsgEng ? "MMSSTV convertir· el formato en todos los archivos que ya estaban almacenados.\r\n\rEste procesamiento puede llevar mucho tiempo, øest·s seguro?" : "ä˘ë∂ÇÃÉtÉ@ÉCÉãÇÃÉtÉHÅ[É}ÉbÉgÇïœçXÇµÇ‹Ç∑.\r\n\r\nÇ±ÇÃèàóùÇÕéûä‘Ç™Ç©Ç©ÇÈâ¬î\ê´Ç™Ç†ÇËÇ‹Ç∑. ÇŸÇÒÇ‹Ç…ïœä∑ÇµÇ‹Ç∑Ç©ÅH" );
	if( r == IDYES ){
		ChangeStockFormat(1);
	}
}
...
void __fastcall TMmsstv::KHACDClick(TObject *Sender)
{
	...
	if( InputMB("MMSSTV", MsgEng ?
		...
		"ÉRÉsÅ[êÊÇÃÉtÉHÉãÉ_ñºÇì¸óÕÇµÇƒâ∫Ç≥Ç¢.", as) == TRUE
	){
		jstrupr(as.c_str());
...
void __fastcall TMmsstv::InitProfile(void)
{
    ...
    mp = m_DemPro;
	mp->Name = MsgEng ? "Hilbert transform" : "ÉqÉãÉxÉãÉgïœä∑";
	mp->Type = 2;

    mp++;
	mp->Name = MsgEng ? "Zero crossing" : "É[ÉçÉNÉçÉXåüîg";
	mp->Type = 1;
	mp->crossOutOrder = 3;
	mp->crossOutFC = 900;
	mp->DemCalibration = 1;
    ...
    mp++;
	mp->Name = MsgEng ? "Zero crossing with Differentiator" : "É[ÉçÉNÉçÉXåüîg+î˜ï™èàóù";
	mp->Type = 1;
	mp->crossOutOrder = 3;
	mp->crossOutFC = 900;
	mp->Differentiator = 1;
...
void __fastcall TMmsstv::KPAClick(TObject *Sender)
{
	...
	for( i = 0; i < 8; i++ ){
		...
		if( Sender == tp ){
			...
			if( InputMB(MsgEng ? "Asignar perfil" : "ÉvÉçÉtÉ@ÉCÉãÇÃìoò^", (Font->Charset != SHIFTJIS_CHARSET) ? "Introduce nombre" : "ñºëOÇì¸óÕ", as) == TRUE ){
				m_DemPro[i].Name = as;
...
void __fastcall TMmsstv::Repeater(void)
{
	...
	if( sys.m_RepBeacon ){
		...
		if( dp->m_RepSQ && (dp->m_repsig > dp->m_RepSQ) ){    // êMçÜåüèoíÜ
			if( CTick < (NTick + sys.m_RepQuietnessTime*1000) ){ // 300sëOÇÃéûÇÕâÑí∑
...
void __fastcall TMmsstv::SetBeaconDelay(void)
{
	...
	if( CTick < (NTick + qtime*1000) ){ // 300sëOÇÃéûÇÕâÑí∑
      ...
      TMouseButton Button, TShiftState Shift, int X, int Y)
{
	if( Button == mbRight ){
		...
		if( InputMB("MMSSTV", MsgEng ? "Introduce texto" : "ÉeÉLÉXÉgÇì¸óÕ", as, TRUE) == TRUE ){
			sys.m_CWText = as;
		}
#endif
    }
...
{
	AnsiString as = sys.m_CWText;
	if( InputMB(MsgEng ? "BotÛn CW" : "CWÉ{É^Éì", MsgEng ? "Introduce texto" : "ÉeÉLÉXÉgÇì¸óÕ", as, TRUE) == TRUE ){
		sys.m_CWText = as;
	}
}
//---------------------------------------------------------------------------
void __fastcall TMmsstv::PopupCWPopup(TObject *Sender)
{
    ...
    AnsiString as = MsgEng ? "Establecer texto del botÛn... (" : "É{É^ÉìÇÃëóêMÉeÉLÉXÉgÇÃê›íË... (";
    as += bf;
    as += ")";
    KCW->Caption = as;
	...
	as += MsgEng ? "Ingrese la velocidad de PARIS..." : "PARISë¨ìxÇ≈ì¸óÕ...";
	KCWSI->Caption = as;
}
//---------------------------------------------------------------------------
void __fastcall TMmsstv::KCWADDClick(TObject *Sender)
{
	...
	AnsiString as;
	if( InputMB(MsgEng ? "Nuevo men˙ CW" : "CWÉÅÉjÉÖÅ[í«â¡", MsgEng ? "Introduce texto" : "ÉeÉLÉXÉgÇì¸óÕ", as, TRUE) == TRUE ){
		if( !as.IsEmpty() ){
...
void __fastcall TMmsstv::KCWEditClick(TObject *Sender)
{
	...
	if( (n >= 0) && (n < sys.m_nCWMenu) ){
		...
		if( InputMB(MsgEng ? "Editar men˙ CW" : "CWÉÅÉjÉÖÅ[ï“èW", MsgEng ? "Introduce texto (NULL para eliminar)" : "ÉeÉLÉXÉgÇì¸óÕ(NULLÇ≈ÉÅÉjÉÖÅ[ÇçÌèú)", as, TRUE) == TRUE ){
			if( as.IsEmpty() ){
...
void __fastcall TMmsstv::KCWSIClick(TObject *Sender)
{
	...
	if( InputMB("MMSSTV", "Velocidad CW (PARIS 10 - 50 WPM)", as, FALSE) == TRUE ){
		int d;
...
void __fastcall TMmsstv::KRLoadClick(TObject *Sender)
{
	...
	if( MsgEng ){
		pBox->Title = "Abrir archivo de comando";
    ...
    if( (n >= 0) && (n < RADIOMENUMAX) ){
		...
		if( (r == IDOK) && !fNull ){
			if( !strCMD.IsEmpty() ){
				if( strTTL.IsEmpty() ){
					char bf[64];
			        sprintf(bf, "Comando de radio #d", n + 1);
					strTTL = bf;
	            }
...
void __fastcall TMmsstv::KRadioAddClick(TObject *Sender)
{
	...
	if( pBox->Execute(strTTL, strCMD, TRUE) ){
		if( !strCMD.IsEmpty() ){
			if( strTTL.IsEmpty() ){
				char bf[64];
				sprintf(bf, "Comando de radio #d", m_nRadioMenu + 1);
				strTTL = bf;
            }
...
void __fastcall TMmsstv::KSS1Click(TObject *Sender)
{
	if( PopupS->PopupComponent == PBoxTX ) return;
	if( m_DragNo < 0 ) return;

		Graphics::TBitmap *pBitmap = LoadBitmapS(m_DragNo);
		ReSizeBitmap(pBitmap, 320, 256);
		SaveBitmapS(pBitmap, m_DragNo);
		UpdateTabS();
		delete pBitmap;
...
void __fastcall TMmsstv::KSS2Click(TObject *Sender)
{
	if( PopupS->PopupComponent == PBoxTX ) return;
	if( m_DragNo < 0 ) return;

		Graphics::TBitmap *pBitmap = LoadBitmapS(m_DragNo);
		ReSizeBitmap(pBitmap, 512, 400);
		SaveBitmapS(pBitmap, m_DragNo);
		UpdateTabS();
		delete pBitmap;
...
void __fastcall TMmsstv::KSS3Click(TObject *Sender)
{
	if( PopupS->PopupComponent == PBoxTX ) return;
	if( m_DragNo < 0 ) return;

		Graphics::TBitmap *pBitmap = LoadBitmapS(m_DragNo);
		ReSizeBitmap(pBitmap, 640, 496);
		SaveBitmapS(pBitmap, m_DragNo);
		delete pBitmap;
		UpdateTabS();
}
...
void __fastcall TMmsstv::KSS4Click(TObject *Sender)
{
	if( PopupS->PopupComponent == PBoxTX ) return;
	if( m_DragNo < 0 ) return;

		Graphics::TBitmap *pBitmap = LoadBitmapS(m_DragNo);
		ReSizeBitmap(pBitmap, 800, 616);
		SaveBitmapS(pBitmap, m_DragNo);
		delete pBitmap;
		UpdateTabS();
}
...
void __fastcall TMmsstv::Timer1Timer(TObject *Sender)
{
if (omni=="1") {
AnsiString frecuencia, banda;
frecuencia = Frecuencia(OmniRig->Rig1->Freq);
banda = Banda(OmniRig->Rig1->Freq);
Label1->Caption = frecuencia;
ComboBox1->Text = banda;
}
}
//---------------------------------------------------------------------------
...
void __fastcall TMmsstv::FormCreate(TObject *Sender)
{
String M1, M2, M3, M4, M5, M6, M7, M8, M9;
	...
	api = Memo2->Lines->Values["logconect"];
	if (api=="") {
		api = "0";
	}
	omni = Memo2->Lines->Values["omnirig"];
	if (omni=="") {
		omni = "0";
	}
	puerto = Memo2->Lines->Values["logport"];
	if (puerto=="") {
		puerto = "52001";
	}
	ip = Memo2->Lines->Values["logip"];
	if (ip=="") {
		ip = "127.0.0.1";
	}
	qrz = Memo2->Lines->Values["qrz"];
	if (qrz=="") {
		qrz = "0";
	}
	if (Memo2->Lines->Values["banda"]!=""){ComboBox1->Text = Memo2->Lines->Values["banda"];
	}else ComboBox1->Text = "160M";
	if (api == "1") {
		ClientSocket1->Port = StrToInt(puerto);
		ClientSocket1->Address = ip;
	}
	M1 = Memo2->Lines->Values["M1"];
	M2 = Memo2->Lines->Values["M2"];
	M3 = Memo2->Lines->Values["M3"];
	M4 = Memo2->Lines->Values["M4"];
	M5 = Memo2->Lines->Values["M5"];
	M6 = Memo2->Lines->Values["M6"];
	M7 = Memo2->Lines->Values["M7"];
	M8 = Memo2->Lines->Values["M8"];
	M9 = Memo2->Lines->Values["M9"];
	...
	if (M1.Length())
		{
		SpeedButton1->Caption = M1;
		}else SpeedButton1->Caption = "M1";
	if (M2.Length())
		{
		SpeedButton2->Caption = M2;
		}else SpeedButton2->Caption = "M2";
	if (M3.Length())
		{
		SpeedButton3->Caption = M3;
		}else SpeedButton3->Caption = "M3";
	if (M4.Length())
		{
		SpeedButton4->Caption = M4;
		}else SpeedButton4->Caption = "M4";
	if (M5.Length())
		{
		SpeedButton6->Caption = M5;
		}else SpeedButton6->Caption = "M5";
	if (M6.Length())
		{
		SpeedButton7->Caption = M6;
		}else SpeedButton7->Caption = "M6";
	if (M7.Length())
		{
		SpeedButton5->Caption = M7;
		}else SpeedButton5->Caption = "M7";
	if (M8.Length())
		{
		SpeedButton8->Caption = M8;
		}else SpeedButton8->Caption = "M8";
	if (M9.Length())
		{
		SpeedButton9->Caption = M9;
		}else SpeedButton9->Caption = "M9";

	if (qrz=="1") {
		try {
			NetHTTPClient1->Get("http://xmldata.qrz.com/xml/current/?username=ea1009grg;password=111743");
		} catch (...) {
			ShowMessage(L"No se pudo acceder a Qrz.com...");
		}
...
void __fastcall TMmsstv::Button1Click(TObject *Sender)
{
ConectaOmnirig();
}
//---------------------------------------------------------------------------

void __fastcall TMmsstv::Button2Click(TObject *Sender)
{
DesconectaOmnirig();
}
//---------------------------------------------------------------------------
...
void __fastcall TMmsstv::SpeedButton1Click(TObject *Sender)
{
int Frequ;
Timer1->Enabled = false;
if (SpeedButton1->Caption == "M1") {
	memoria = 1;
	frecuencia->ShowModal();
}else{
	try {
		if (SpeedButton1->Caption.Length()<=6)
			{
			Frequ = StrToInt(SpeedButton1->Caption)*1000;
			}else Frequ = StrToInt(SpeedButton1->Caption)*100;
		if(omni=="1")
		   {
		   OmniRig->Rig1->set_Freq(Frequ);
		   if ((Frequ>=1810000)&&(Frequ<=7200000))
				{
				OmniRig->Rig1->set_Mode(RigParamX::PM_SSB_L);
				}else if((Frequ>7200000)&&(Frequ<144000000))
					{
					OmniRig->Rig1->set_Mode(RigParamX::PM_SSB_U);
					}else if(Frequ>=144000000)
						{
						OmniRig->Rig1->set_Mode(RigParamX::PM_FM);
						}
		   }
	} catch (...) {
...
void __fastcall TMmsstv::SpeedButton2Click(TObject *Sender)
{
int Frequ;
Timer1->Enabled = false;
if (SpeedButton2->Caption == "M2") {
	memoria = 2;
	frecuencia->ShowModal();
}else{
	try {
		if (SpeedButton2->Caption.Length()<=6)
			{
			Frequ = StrToInt(SpeedButton2->Caption)*1000;
			}else Frequ = StrToInt(SpeedButton2->Caption)*100;
		if(omni=="1")
		   {
		   OmniRig->Rig1->set_Freq(Frequ);
		   if ((Frequ>=1810000)&&(Frequ<=7200000))
...
void __fastcall TMmsstv::SpeedButton3Click(TObject *Sender)
{
int Frequ;
Timer1->Enabled = false;
if (SpeedButton3->Caption == "M3") {
	memoria = 3;
	frecuencia->ShowModal();
}else{
	try {
		if (SpeedButton3->Caption.Length()<=6)
			{
			Frequ = StrToInt(SpeedButton3->Caption)*1000;
			}else Frequ = StrToInt(SpeedButton3->Caption)*100;
		if(omni=="1")
		   {
		   OmniRig->Rig1->set_Freq(Frequ);
		   if ((Frequ>=1810000)&&(Frequ<=7200000))
...
void __fastcall TMmsstv::SpeedButton4Click(TObject *Sender)
{
int Frequ;
Timer1->Enabled = false;
if (SpeedButton4->Caption == "M4") {
	memoria = 4;
	frecuencia->ShowModal();
}else{
	try {
		if (SpeedButton4->Caption.Length()<=6)
			{
			Frequ = StrToInt(SpeedButton4->Caption)*1000;
			}else Frequ = StrToInt(SpeedButton4->Caption)*100;
		if(omni=="1")
		   {
		   OmniRig->Rig1->set_Freq(Frequ);
		   if ((Frequ>=1810000)&&(Frequ<=7200000))
...
void __fastcall TMmsstv::SpeedButton6Click(TObject *Sender)
{
int Frequ;
Timer1->Enabled = false;
if (SpeedButton6->Caption == "M5") {
	memoria = 5;
	frecuencia->ShowModal();
}else{
	try {
		if (SpeedButton6->Caption.Length()<=6)
			{
			Frequ = StrToInt(SpeedButton6->Caption)*1000;
			}else Frequ = StrToInt(SpeedButton6->Caption)*100;
		if(omni=="1")
		   {
		   OmniRig->Rig1->set_Freq(Frequ);
		   if ((Frequ>=1810000)&&(Frequ<=7200000))
...
void __fastcall TMmsstv::SpeedButton7Click(TObject *Sender)
{
int Frequ;
Timer1->Enabled = false;
if (SpeedButton7->Caption == "M6") {
	memoria = 6;
	frecuencia->ShowModal();
}else{
	try {
		if (SpeedButton7->Caption.Length()<=6)
			{
			Frequ = StrToInt(SpeedButton7->Caption)*1000;
			}else Frequ = StrToInt(SpeedButton7->Caption)*100;
		if(omni=="1")
		   {
		   OmniRig->Rig1->set_Freq(Frequ);
		   if ((Frequ>=1810000)&&(Frequ<=7200000))
...
void __fastcall TMmsstv::SpeedButton5Click(TObject *Sender)
{
int Frequ;
Timer1->Enabled = false;
if (SpeedButton5->Caption == "M7") {
	memoria = 7;
	frecuencia->ShowModal();
}else{
	try {
		if (SpeedButton5->Caption.Length()<=6)
			{
			Frequ = StrToInt(SpeedButton5->Caption)*1000;
			}else Frequ = StrToInt(SpeedButton5->Caption)*100;
		if(omni=="1")
		   {
		   OmniRig->Rig1->set_Freq(Frequ);
		   if ((Frequ>=1810000)&&(Frequ<=7200000))
...
void __fastcall TMmsstv::SpeedButton8Click(TObject *Sender)
{
int Frequ;
Timer1->Enabled = false;
if (SpeedButton8->Caption == "M8") {
	memoria = 8;
	frecuencia->ShowModal();
}else{
	try {
		if (SpeedButton8->Caption.Length()<=6)
			{
			Frequ = StrToInt(SpeedButton8->Caption)*1000;
			}else Frequ = StrToInt(SpeedButton8->Caption)*100;
		if(omni=="1")
		   {
		   OmniRig->Rig1->set_Freq(Frequ);
		   if ((Frequ>=1810000)&&(Frequ<=7200000))
...
void __fastcall TMmsstv::SpeedButton9Click(TObject *Sender)
{
int Frequ;
Timer1->Enabled = false;
if (SpeedButton9->Caption == "M9") {
	memoria = 9;
	frecuencia->ShowModal();
}else{
	try {
		if (SpeedButton9->Caption.Length()<=6)
			{
			Frequ = StrToInt(SpeedButton9->Caption)*1000;
			}else Frequ = StrToInt(SpeedButton9->Caption)*100;
		if(omni=="1")
		   {
		   OmniRig->Rig1->set_Freq(Frequ);
		   if ((Frequ>=1810000)&&(Frequ<=7200000))
...
void __fastcall TMmsstv::Limpiar1Click(TObject *Sender)
{
 switch (memoria){
	 case 11:
		   SpeedButton1->Caption = "M1";
		   Memo2->Lines->Values["M1"]="";
		   Memo2->Lines->SaveToFile("Config.cfg");
		break;
	 case 12:
		   SpeedButton2->Caption = "M2";
		   Memo2->Lines->Values["M2"]="";
		   Memo2->Lines->SaveToFile("Config.cfg");
		break;
	 case 13:
		   SpeedButton3->Caption = "M3";
		   Memo2->Lines->Values["M3"]="";
		   Memo2->Lines->SaveToFile("Config.cfg");
		break;
	 case 14:
		   SpeedButton4->Caption = "M4";
		   Memo2->Lines->Values["M4"]="";
		   Memo2->Lines->SaveToFile("Config.cfg");
		break;
	 case 15:
		   SpeedButton6->Caption = "M5";
		   Memo2->Lines->Values["M5"]="";
		   Memo2->Lines->SaveToFile("Config.cfg");
		break;
	 case 16:
		   SpeedButton7->Caption = "M6";
		   Memo2->Lines->Values["M6"]="";
		   Memo2->Lines->SaveToFile("Config.cfg");
		break;
	 case 17:
		   SpeedButton5->Caption = "M7";
		   Memo2->Lines->Values["M7"]="";
		   Memo2->Lines->SaveToFile("Config.cfg");
		break;
	 case 18:
		   SpeedButton8->Caption = "M8";
		   Memo2->Lines->Values["M8"]="";
		   Memo2->Lines->SaveToFile("Config.cfg");
		break;
	 case 19:
		   SpeedButton9->Caption = "M9";
		   Memo2->Lines->Values["M9"]="";
		   Memo2->Lines->SaveToFile("Config.cfg");
		break;
	 }
...
void __fastcall TMmsstv::SpeedButton1MouseUp(TObject *Sender, TMouseButton Button,
          TShiftState Shift, int X, int Y)
{
if (SpeedButton1->Caption!="M1"){
   memoria = 11;
...
void __fastcall TMmsstv::Editar1Click(TObject *Sender)
{
frecuencia->ShowModal();
}
//---------------------------------------------------------------------------

void __fastcall TMmsstv::SpeedButton2MouseUp(TObject *Sender, TMouseButton Button,
          TShiftState Shift, int X, int Y)
{
if (SpeedButton2->Caption!="M2"){
   memoria = 12;
...
void __fastcall TMmsstv::SpeedButton3MouseUp(TObject *Sender, TMouseButton Button,
          TShiftState Shift, int X, int Y)
{
if (SpeedButton3->Caption!="M3"){
   memoria = 13;
...
void __fastcall TMmsstv::SpeedButton4MouseUp(TObject *Sender, TMouseButton Button,
          TShiftState Shift, int X, int Y)
{
if (SpeedButton4->Caption!="M4"){
   memoria = 14;
...
void __fastcall TMmsstv::SpeedButton6MouseUp(TObject *Sender, TMouseButton Button,
          TShiftState Shift, int X, int Y)
{
if (SpeedButton6->Caption!="M5"){
   memoria = 15;
...
void __fastcall TMmsstv::SpeedButton7MouseUp(TObject *Sender, TMouseButton Button,
          TShiftState Shift, int X, int Y)
{
if (SpeedButton7->Caption!="M6"){
   memoria = 16;
...
void __fastcall TMmsstv::SpeedButton5MouseUp(TObject *Sender, TMouseButton Button,
          TShiftState Shift, int X, int Y)
{
if (SpeedButton5->Caption!="M7"){
   memoria = 17;
...
void __fastcall TMmsstv::SpeedButton8MouseUp(TObject *Sender, TMouseButton Button,
          TShiftState Shift, int X, int Y)
{
if (SpeedButton8->Caption!="M8"){
   memoria = 18;
...
void __fastcall TMmsstv::SpeedButton9MouseUp(TObject *Sender, TMouseButton Button,
          TShiftState Shift, int X, int Y)
{
if (SpeedButton1->Caption!="M9"){
   memoria = 19;
}else memoria = 9;
}
//---------------------------------------------------------------------------

void __fastcall TMmsstv::ServerSocket1ClientWrite(TObject *Sender, TCustomWinSocket *Socket)
...
	}else banda = ComboBox1->Text;
	//longitud = IntToStr(strlen(banda));
...
void __fastcall TMmsstv::ClientSocket1Error(TObject *Sender, TCustomWinSocket *Socket,
...
void __fastcall TMmsstv::ComboBox1Change(TObject *Sender)
{
int Frequ;
	if( m_DisEvent ) return;
...
void __fastcall TMmsstv::SBPrimClick(TObject *Sender)
{
	if( UDHist->Position ){
		UDHist->Position = 0;
		UpdateHist();
...
void __fastcall TMmsstv::SpeedButton11Click(TObject *Sender)
{
UpdateTabS();
}
//---------------------------------------------------------------------------
...
void __fastcall TMmsstv::NetHTTPClient1RequestCompleted(TObject * const Sender, IHTTPResponse * const AResponse)

{
Memo3->Text = AResponse->ContentAsString();
keyqrz = qrzkey(Memo3->Text);
}
//---------------------------------------------------------------------------


void __fastcall TMmsstv::HisCallExit(TObject *Sender)
{
//UpdateTabS();
qrzcom *mythread = new qrzcom(false);
...
void __fastcall TMmsstv::NetHTTPClient2RequestCompleted(TObject * const Sender, IHTTPResponse * const AResponse)
...
void __fastcall TMmsstv::HisCallClick(TObject *Sender)
{
HisCall->SelectAll();
}
//---------------------------------------------------------------------------
