	...
	: TForm(AOwner)
{
	...
	if( MsgEng ){
		...
		L1->Caption = "Brillo";
		L2->Caption = "Contraste";
		L3->Caption = "Definición";
		L4->Caption = "Hue";
		L5->Caption = "Saturación";
		GBFilt->Caption = "Filtro de ruido";
		SBST->Caption = "Estirar";
		AutoBtn->Caption = "Auto";
		ResBtn->Caption = "Reiniciar";
		SBEdit->Caption = "Editar";
		SBBRR->Hint = "Reiniciar";
		SBCTR->Hint = "Reiniciar";
		SBSPR->Hint = "Reiniciar";
		SBCPR->Hint = "Reiniciar";
		SBCOR->Hint = "Reiniciar";
	}
	else {
		SBBRR->Hint = "‰ðœ";
		SBCTR->Hint = "‰ðœ";
...
int __fastcall TPicFilterDlg::Execute(Graphics::TBitmap *pBmp)
{
	...
	m_YW = pBmp->Height;
	char bf[128];
	sprintf(bf, "%s - Fuente:%ux%u", MsgEng ? "Ajuste de imagen" : "‰æ‘œ’²®", m_XW, m_YW);
	Caption = bf;
	pBitmap->Width = m_XW;
	pBitmap->Height = m_YW;
	pBitmapS->Width = m_XW;
