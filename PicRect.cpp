	...
	: TForm(AOwner)
{
	...
	if( MsgEng ){
		...
		Label1->Caption = "Estirar";
		SBRatio->Hint = "Mantener aspecto";
		SBStrach->Hint = "Estirar";
		SBNStrach->Hint = "No estirar";

		SBRL->Hint = "Rotar a la iquierda";
		SBRR->Hint = "Rotar a la derecha";
		SBMIR->Hint = "Rotar a derecha e izquierda";

		KFS->Caption = "&Guardar...";
		KFSC->Caption = "Guardar imagen de recorte...";
		KEC->Caption = "&Copiar";
		KECC->Caption = "Copiar imagen de recorte";
		KVAS->Caption = "Mostrar imagen y tamaño real";

		KEFC->Caption = "Seleccionar color";
		KEFCP->Caption = "Punto";
		KEFCM->Caption = "Manual...";
		KEE->Caption = "&Editar imagen...";
		KEF->Caption = "Espacio";
		KEFD->Caption = "Borrar espacio";
		KER->Caption = "Rotación...";
		KEMC->Caption = "Centrar";
		KEMM->Caption = "Recorte por rango máximo";
	}
...
void __fastcall TPicRectDlg::UpdateTitle(void)
{
	...
	sprintf(bf, "%s - Fuente:%ux%u,  Objetivo:%ux%u", MsgEng ? "Imagen clíper" : "‰æ‘œƒNƒŠƒbƒp[", pBitmap->Width, pBitmap->Height, pwBmp->Width, h);
	Caption = bf;
}
//---------------------------------------------------------------------
...
void __fastcall TPicRectDlg::PBoxPaint(TObject *Sender)
{
    ...
    if( m_RotFlag ){
		...
		PBox->Canvas->TextOut(0, 0, MsgEng ? "[Rotación] Dibuja una línea base" : "[‰ñ“]] ƒx[ƒXƒ‰ƒCƒ“‚ðˆø‚¢‚Ä‚­‚¾‚³‚¢");
    }
}
//---------------------------------------------------------------------------
...
{
	Mmsstv->SaveBitmapMenu(pBitmap, "Imagen", NULL);
}
//---------------------------------------------------------------------------
void __fastcall TPicRectDlg::KFSCClick(TObject *Sender)
{
	Graphics::TBitmap *pBmp = GetClipBitmap();
	Mmsstv->SaveBitmapMenu(pBmp, "Imagen", NULL);
	delete pBmp;
}
//---------------------------------------------------------------------------
...
{
	AnsiString as;
	if( InputMB(NULL, MsgEng ? "Ingrese el grado de rotación" : "‰ñ“]Šp(‹)‚ð“ü—Í", as) == IDOK ){
		double deg;
