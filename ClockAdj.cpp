...
void __fastcall TClockAdjDlg::FormResize(TObject *Sender)
{
	...
	int Y = 5;
	if( MsgEng ){
		Caption = "Calibración de la tarjeta de sonido con una estación de transmisión estándar de tiempo";
...
//		Caption = "Adjust Sampling freq. (Recive WWV tick sound on Mark freq.)";
		...
		cp->TextOut(10, Y, "1) Recibe ondas de radio estándar (e.g., WWV, BPM)."); Y+=FH;
		cp->TextOut(10, Y, "2) Sintonice el sonido del tic."); Y+=FH;
		cp->TextOut(10, Y, "3) Continúa escuchando el sonido por un tiempo. Tienes una línea vertical."); Y+=FH;
		cp->TextOut(10, Y, "4) Haga clic en el punto inferior de la línea.."); Y+=FH;
		cp->TextOut(10, Y, "5) Haga clic en el punto superior de la línea.."); Y+=FH;
		Y+=FH;
		cp->TextOut(10, Y, "Puede usar la transmisión de FAX en lugar de WWV o JJY, pero asegúrese de que tenga"); Y+=FH;
		cp->TextOut(10, Y, "tiempo exacto.");
	}
	else {
		cp->TextOut(10, Y, "1.BPM‚ðAM‚Ü‚½‚ÍSSB‚ÅŽóM‚µA‚±‚Ì‰æ–Ê‚Ì‰º‚É‚ ‚éTone‚É1000Hz‚Ü‚½‚Í"); Y+=FH;
		cp->TextOut(10, Y, "  1600Hz‚ðÝ’è‚µ‚Ü‚·."); Y+=FH;
		cp->TextOut(10, Y, "2.SSB‚Ìê‡‚Í‚P•bƒ`ƒbƒN‰¹‚ªTone‚Éd‚È‚é‚æ‚¤‚ÉŽóMŽü”g”‚ð’²®‚µ‚Ü‚·."); Y+=FH;
		cp->TextOut(10, Y, "3.‚µ‚Î‚ç‚­ŽóM‚µ‚Äc‚Ü‚½‚ÍŽÎ‚ß‚Ì‘Ñü‚ª•\Ž¦‚³‚ê‚é‚Ì‚ð‘Ò‚¿‚Ü‚·i”•ªj."); Y+=FH;
		cp->TextOut(10, Y, "4.•\Ž¦‚³‚ê‚½‘Ñü‚Ì‰º‚Ì’[“_‚ðƒNƒŠƒbƒN‚µ‚Ü‚·."); Y+=FH;
		cp->TextOut(10, Y, "5.X‚É“¯‚¶‘Ñü‚Ìã‚Ì’[“_‚ðƒNƒŠƒbƒN‚·‚é‚ÆƒNƒƒbƒN’l‚ªŽ©“®“I‚É"); Y+=FH;
		cp->TextOut(10, Y, "  Ý’è‚³‚ê‚Ü‚·.i2“_ŠÔ‚Ì‹——£‚ª’·‚¢‚Ù‚¤‚ª³Šm‚Å‚·j"); Y+=FH;
		cp->TextOut(10, Y, "ŒXŽÎ‚ª”ñí‚É‘å‚«‚¢ê‡Aã‹L‚ÌŽè‘±‚«‚ð‚à‚¤ˆê“xŒJ‚è•Ô‚µ‚Ä‰º‚³‚¢."); Y+=FH+FH;
		cp->TextOut(10, Y, "BPM‚Ì‘ã‚í‚è‚ÉAJMH,JMG‚È‚Ç‚ÌFAX•ú‘—‚Ì0.5•b–ˆ‚ÌƒfƒbƒhƒZƒNƒ^‚ð—˜—p‚µ‚Ä‚à"); Y+=FH;
		cp->TextOut(10, Y, "OK‚Ì‚æ‚¤‚Å‚·."); Y+=FH;
		cp->TextOut(10, Y, "¶ƒNƒŠƒbƒN - ’²®‚ÌŠJŽn"); Y+=FH;
		cp->TextOut(10, Y, "‰EƒNƒŠƒbƒN - ‚’¼ƒJ[ƒ\ƒ‹ˆÚ“®");
	}
}
//---------------------------------------------------------------------------
...
void __fastcall TClockAdjDlg::PaintCursor(void)
{
	...
	if( m_Point ){
		...
		char bf[256];
		if( MsgEng ){
			sprintf( bf, "OK : Botón izquierdo,  Cancelar : Botón derecho   Clock=%.2lf", GetPointSamp());
		}
		else {
			sprintf( bf, "Œˆ’è : ¶ƒ{ƒ^ƒ“,  ’†Ž~ : ‰Eƒ{ƒ^ƒ“   Clock=%.2lf", GetPointSamp());
		}
		DrawMessage(bf);
